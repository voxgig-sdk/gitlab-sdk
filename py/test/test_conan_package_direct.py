...
class TestConanPackageDirect:

    def test_should_direct_load_conan_package(self):
        setup = _conan_package_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-conan_package", "live" if setup["live"] else "unit")
        if _skip:
...
def _conan_package_direct_setup(mockres):
