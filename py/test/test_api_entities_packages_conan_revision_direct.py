...
class TestApiEntitiesPackagesConanRevisionDirect:

    def test_should_direct_load_api_entities_packages_conan_revision(self):
        setup = _api_entities_packages_conan_revision_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-api_entities_packages_conan_revision", "live" if setup["live"] else "unit")
        if _skip:
...
def _api_entities_packages_conan_revision_direct_setup(mockres):
