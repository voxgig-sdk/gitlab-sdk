...
class TestApiEntitiesBranchEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ApiEntitiesBranch(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ApiEntitiesBranch(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ApiEntitiesBranch(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _api_entities_branch_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "api_entities_branch." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_API_ENTITIES_BRANCH_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        api_entities_branch_ref01_ent = client.ApiEntitiesBranch(None)
        ...
        api_entities_branch_ref01_list_result = api_entities_branch_ref01_ent.list(api_entities_branch_ref01_match, None)
        assert isinstance(api_entities_branch_ref01_list_result, list)
...
def _api_entities_branch_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/api_entities_branch/ApiEntitiesBranchTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_API_ENTITIES_BRANCH_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_API_ENTITIES_BRANCH_ENTID"))
