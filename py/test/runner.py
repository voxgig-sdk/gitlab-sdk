...
class GitlabTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GitlabTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GitlabTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GitlabTestRunner.getenv("GITLAB_TEST_LIVE")
        override = GitlabTestRunner.getenv("GITLAB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GitlabTestRunner.getenv(key)
        ...
        explain = GitlabTestRunner.getenv("GITLAB_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GITLAB_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GitlabTestRunner._test_control is not None:
            return GitlabTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GitlabTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GitlabTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GitlabTestRunner.load_test_control()
...
def load_env_local():
    GitlabTestRunner.load_env_local()


def env_override(m):
    return GitlabTestRunner.env_override(m)


def entity_data(v):
    return GitlabTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GitlabTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GitlabTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GitlabTestRunner.load_test_control()


def live_client_options():
    return GitlabTestRunner.live_client_options()


def live_delay_ms():
    return GitlabTestRunner.live_delay_ms()
