...
class TestApiEntitiesCommitDirect:

    def test_should_direct_list_api_entities_commit(self):
        setup = _api_entities_commit_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_commit", "live" if setup["live"] else "unit")
        if _skip:
...
def _api_entities_commit_direct_setup(mockres):
