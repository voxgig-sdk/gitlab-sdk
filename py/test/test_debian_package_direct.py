...
class TestDebianPackageDirect:

    def test_should_direct_load_debian_package(self):
        setup = _debian_package_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-debian_package", "live" if setup["live"] else "unit")
        if _skip:
...
def _debian_package_direct_setup(mockres):
