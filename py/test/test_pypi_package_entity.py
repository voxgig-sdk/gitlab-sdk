...
class TestPypiPackageEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.PypiPackage(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _pypi_package_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "pypi_package." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_PYPI_PACKAGE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        pypi_package_ref01_ent = client.PypiPackage(None)
        ...
        pypi_package_ref01_data = helpers.to_map(runner.entity_data(pypi_package_ref01_ent.create(pypi_package_ref01_data, None)))
        assert pypi_package_ref01_data is not None

        # LOAD
        pypi_package_ref01_match_dt0 = {}
        pypi_package_ref01_data_dt0_loaded = pypi_package_ref01_ent.load(pypi_package_ref01_match_dt0, None)
        assert pypi_package_ref01_data_dt0_loaded is not None



def _pypi_package_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/pypi_package/PypiPackageTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_PYPI_PACKAGE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_PYPI_PACKAGE_ENTID"))
