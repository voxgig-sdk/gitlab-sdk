...
class TestApiEntitiesErrorTrackingProjectSettingEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ApiEntitiesErrorTrackingProjectSetting(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _api_entities_error_tracking_project_setting_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "api_entities_error_tracking_project_setting." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_API_ENTITIES_ERROR_TRACKING_PROJECT_SETTING_ENTID JSON to run live")
        client = setup["client"]
        ...
        api_entities_error_tracking_project_setting_ref01_data = None
        if len(api_entities_error_tracking_project_setting_ref01_data_raw) > 0:
        ...
        # UPDATE
        api_entities_error_tracking_project_setting_ref01_ent = client.ApiEntitiesErrorTrackingProjectSetting(None)
        ...
        api_entities_error_tracking_project_setting_ref01_data_dt0_loaded = api_entities_error_tracking_project_setting_ref01_ent.load(api_entities_error_tracking_project_setting_ref01_match_dt0, None)
        assert api_entities_error_tracking_project_setting_ref01_data_dt0_loaded is not None



def _api_entities_error_tracking_project_setting_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/api_entities_error_tracking_project_setting/ApiEntitiesErrorTrackingProjectSettingTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_API_ENTITIES_ERROR_TRACKING_PROJECT_SETTING_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_API_ENTITIES_ERROR_TRACKING_PROJECT_SETTING_ENTID"))
