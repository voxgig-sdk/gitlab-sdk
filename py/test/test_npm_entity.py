...
class TestNpmEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Npm(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _npm_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "npm." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_NPM_ENTID JSON to run live")
        client = setup["client"]
        ...
        npm_ref01_resdata_up0 = helpers.to_map(npm_ref01_ent.update(npm_ref01_data_up0_up, None))
        assert npm_ref01_resdata_up0 is not None



def _npm_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/npm/NpmTestData.json")
