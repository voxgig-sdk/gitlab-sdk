...
class TestApiEntitiesPackagesConanRecipeRevisionDirect:

    def test_should_direct_list_api_entities_packages_conan_recipe_revision(self):
        setup = _api_entities_packages_conan_recipe_revision_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_packages_conan_recipe_revision", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["conan01", "package_channel01", "package_username01", "package_version01", "project01"]:
...
def _api_entities_packages_conan_recipe_revision_direct_setup(mockres):
