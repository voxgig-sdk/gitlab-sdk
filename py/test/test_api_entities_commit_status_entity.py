...
class TestApiEntitiesCommitStatusEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ApiEntitiesCommitStatus(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ApiEntitiesCommitStatus(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ApiEntitiesCommitStatus(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _api_entities_commit_status_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "api_entities_commit_status." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_API_ENTITIES_COMMIT_STATUS_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        api_entities_commit_status_ref01_ent = client.ApiEntitiesCommitStatus(None)
        ...
        api_entities_commit_status_ref01_list_result = api_entities_commit_status_ref01_ent.list(api_entities_commit_status_ref01_match, None)
        assert isinstance(api_entities_commit_status_ref01_list_result, list)
...
def _api_entities_commit_status_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/api_entities_commit_status/ApiEntitiesCommitStatusTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_API_ENTITIES_COMMIT_STATUS_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_API_ENTITIES_COMMIT_STATUS_ENTID"))
