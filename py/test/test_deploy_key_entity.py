...
class TestDeployKeyEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.DeployKey(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _deploy_key_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "deploy_key." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_DEPLOY_KEY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        deploy_key_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.deploy_key")))
        deploy_key_ref01_data = None
        if len(deploy_key_ref01_data_raw) > 0:
            deploy_key_ref01_data = helpers.to_map(deploy_key_ref01_data_raw[0][1])

        # REMOVE
        deploy_key_ref01_ent = client.DeployKey(None)
        deploy_key_ref01_match_rm0 = {
            "id": deploy_key_ref01_data["id"],
        }
        deploy_key_ref01_ent.remove(deploy_key_ref01_match_rm0, None)



def _deploy_key_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/deploy_key/DeployKeyTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_DEPLOY_KEY_ENTID"))
