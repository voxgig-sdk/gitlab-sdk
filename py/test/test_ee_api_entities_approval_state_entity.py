...
class TestEeApiEntitiesApprovalStateEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.EeApiEntitiesApprovalState(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _ee_api_entities_approval_state_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "ee_api_entities_approval_state." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_EE_API_ENTITIES_APPROVAL_STATE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        ee_api_entities_approval_state_ref01_ent = client.EeApiEntitiesApprovalState(None)
        ...
        ee_api_entities_approval_state_ref01_data = helpers.to_map(runner.entity_data(ee_api_entities_approval_state_ref01_ent.create(ee_api_entities_approval_state_ref01_data, None)))
        assert ee_api_entities_approval_state_ref01_data is not None



def _ee_api_entities_approval_state_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/ee_api_entities_approval_state/EeApiEntitiesApprovalStateTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_EE_API_ENTITIES_APPROVAL_STATE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_EE_API_ENTITIES_APPROVAL_STATE_ENTID"))
