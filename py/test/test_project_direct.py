...
class TestProjectDirect:
    ...
    def test_should_direct_list_project(self):
        setup = _project_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-project", "live" if setup["live"] else "unit")
        if _skip:
