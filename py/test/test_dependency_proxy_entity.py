...
class TestDependencyProxyEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.DependencyProxy(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _dependency_proxy_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_DEPENDENCY_PROXY_ENTID JSON to run live")
        client = setup["client"]
...
def _dependency_proxy_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/dependency_proxy/DependencyProxyTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_DEPENDENCY_PROXY_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_DEPENDENCY_PROXY_ENTID"))
