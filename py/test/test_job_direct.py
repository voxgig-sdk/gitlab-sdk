...
class TestJobDirect:
    ...
    def test_should_direct_load_job(self):
        setup = _job_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-job", "live" if setup["live"] else "unit")
        if _skip:
