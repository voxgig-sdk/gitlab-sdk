...
class TestPackageFileEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.PackageFile(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _package_file_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_PACKAGE_FILE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        package_file_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.package_file")))
        package_file_ref01_data = None
        if len(package_file_ref01_data_raw) > 0:
...
def _package_file_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/package_file/PackageFileTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_PACKAGE_FILE_ENTID"))
