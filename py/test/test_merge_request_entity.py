...
class TestMergeRequestEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.MergeRequest(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _merge_request_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "merge_request." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_MERGE_REQUEST_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        merge_request_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.merge_request")))
        merge_request_ref01_data = None
        if len(merge_request_ref01_data_raw) > 0:
        ...
        merge_request_ref01_resdata_up0 = helpers.to_map(merge_request_ref01_ent.update(merge_request_ref01_data_up0_up, None))
        ...
        merge_request_ref01_ent.remove(merge_request_ref01_match_rm0, None)



def _merge_request_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/merge_request/MergeRequestTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_MERGE_REQUEST_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_MERGE_REQUEST_ENTID"))
