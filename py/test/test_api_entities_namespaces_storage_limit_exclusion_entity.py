...
class TestApiEntitiesNamespacesStorageLimitExclusionEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ApiEntitiesNamespacesStorageLimitExclusion(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _api_entities_namespaces_storage_limit_exclusion_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "api_entities_namespaces_storage_limit_exclusion." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_API_ENTITIES_NAMESPACES_STORAGE_LIMIT_EXCLUSION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        api_entities_namespaces_storage_limit_exclusion_ref01_ent = client.ApiEntitiesNamespacesStorageLimitExclusion(None)
...
def _api_entities_namespaces_storage_limit_exclusion_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/api_entities_namespaces_storage_limit_exclusion/ApiEntitiesNamespacesStorageLimitExclusionTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_API_ENTITIES_NAMESPACES_STORAGE_LIMIT_EXCLUSION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_API_ENTITIES_NAMESPACES_STORAGE_LIMIT_EXCLUSION_ENTID"))
