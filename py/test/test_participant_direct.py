...
class TestParticipantDirect:

    def test_should_direct_list_participant(self):
        setup = _participant_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-participant", "live" if setup["live"] else "unit")
        if _skip:
...
def _participant_direct_setup(mockres):
