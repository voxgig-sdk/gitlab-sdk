...
class TestInvitationEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Invitation(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _invitation_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_INVITATION_ENTID JSON to run live")
        client = setup["client"]
...
def _invitation_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/invitation/InvitationTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_INVITATION_ENTID"))
