...
class TestApiEntitiesProjectWithAccessEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ApiEntitiesProjectWithAccess(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _api_entities_project_with_access_basic_setup(None)
        ...
        api_entities_project_with_access_ref01_data_dt0_loaded = api_entities_project_with_access_ref01_ent.load(api_entities_project_with_access_ref01_match_dt0, None)
        api_entities_project_with_access_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(api_entities_project_with_access_ref01_data_dt0_loaded))
        assert api_entities_project_with_access_ref01_data_dt0_load_result is not None
        assert api_entities_project_with_access_ref01_data_dt0_load_result["id"] == api_entities_project_with_access_ref01_data["id"]



def _api_entities_project_with_access_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/api_entities_project_with_access/ApiEntitiesProjectWithAccessTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_API_ENTITIES_PROJECT_WITH_ACCESS_ENTID"))
