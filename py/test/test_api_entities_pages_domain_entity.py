...
class TestApiEntitiesPagesDomainEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ApiEntitiesPagesDomain(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ApiEntitiesPagesDomain(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ApiEntitiesPagesDomain(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _api_entities_pages_domain_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "api_entities_pages_domain." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_API_ENTITIES_PAGES_DOMAIN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        api_entities_pages_domain_ref01_ent = client.ApiEntitiesPagesDomain(None)
        ...
        assert api_entities_pages_domain_ref01_data["id"] is not None
        ...
        assert api_entities_pages_domain_ref01_resdata_up0["id"] == api_entities_pages_domain_ref01_data_up0_up["id"]
        ...
        api_entities_pages_domain_ref01_data_dt0_loaded = api_entities_pages_domain_ref01_ent.load(api_entities_pages_domain_ref01_match_dt0, None)
        api_entities_pages_domain_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(api_entities_pages_domain_ref01_data_dt0_loaded))
        assert api_entities_pages_domain_ref01_data_dt0_load_result is not None
        assert api_entities_pages_domain_ref01_data_dt0_load_result["id"] == api_entities_pages_domain_ref01_data["id"]



def _api_entities_pages_domain_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/api_entities_pages_domain/ApiEntitiesPagesDomainTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_API_ENTITIES_PAGES_DOMAIN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_API_ENTITIES_PAGES_DOMAIN_ENTID"))
