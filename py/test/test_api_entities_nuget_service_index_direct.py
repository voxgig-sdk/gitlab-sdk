...
class TestApiEntitiesNugetServiceIndexDirect:

    def test_should_direct_list_api_entities_nuget_service_index(self):
        setup = _api_entities_nuget_service_index_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_nuget_service_index", "live" if setup["live"] else "unit")
        if _skip:
...
def _api_entities_nuget_service_index_direct_setup(mockres):
