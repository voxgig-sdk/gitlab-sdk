...
class TestStarrerDirect:

    def test_should_direct_list_starrer(self):
        setup = _starrer_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-starrer", "live" if setup["live"] else "unit")
        if _skip:
...
def _starrer_direct_setup(mockres):
