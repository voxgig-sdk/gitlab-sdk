...
class TestEeApiEntitiesMergeRequestApprovalStateDirect:

    def test_should_direct_list_ee_api_entities_merge_request_approval_state(self):
        setup = _ee_api_entities_merge_request_approval_state_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ee_api_entities_merge_request_approval_state", "live" if setup["live"] else "unit")
        if _skip:
...
def _ee_api_entities_merge_request_approval_state_direct_setup(mockres):
