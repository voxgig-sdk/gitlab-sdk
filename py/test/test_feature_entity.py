...
class TestFeatureEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Feature(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _feature_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_FEATURE_ENTID JSON to run live")
        client = setup["client"]
...
def _feature_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/feature/FeatureTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_FEATURE_ENTID"))
