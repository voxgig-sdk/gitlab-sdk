...
class TestDebianPackageEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.DebianPackage(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _debian_package_basic_setup(None)
        ...
        debian_package_ref01_resdata_up0 = helpers.to_map(debian_package_ref01_ent.update(debian_package_ref01_data_up0_up, None))
...
def _debian_package_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/debian_package/DebianPackageTestData.json")
