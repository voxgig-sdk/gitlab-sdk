...
class TestConanPackageEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ConanPackage(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _conan_package_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_CONAN_PACKAGE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        conan_package_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.conan_package")))
        conan_package_ref01_data = None
        if len(conan_package_ref01_data_raw) > 0:
        ...
        conan_package_ref01_resdata_up0 = helpers.to_map(conan_package_ref01_ent.update(conan_package_ref01_data_up0_up, None))
...
def _conan_package_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/conan_package/ConanPackageTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_CONAN_PACKAGE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)
    if idmap_resolved.get("file_id") is None:
        idmap_resolved["file_id"] = idmap_resolved.get("file01")
    if idmap_resolved.get("package_channel") is None:
        idmap_resolved["package_channel"] = idmap_resolved.get("package_channel01")
    if idmap_resolved.get("package_username") is None:
        idmap_resolved["package_username"] = idmap_resolved.get("package_username01")
    if idmap_resolved.get("package_version") is None:
        idmap_resolved["package_version"] = idmap_resolved.get("package_version01")
    if idmap_resolved.get("recipe_revision") is None:
        idmap_resolved["recipe_revision"] = idmap_resolved.get("recipe_revision01")
