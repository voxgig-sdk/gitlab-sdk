...
class TestProjectsJobTokenScopeEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.ProjectsJobTokenScope(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _projects_job_token_scope_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_PROJECTS_JOB_TOKEN_SCOPE_ENTID JSON to run live")
        client = setup["client"]
        ...
        projects_job_token_scope_ref01_data = None
        if len(projects_job_token_scope_ref01_data_raw) > 0:
            projects_job_token_scope_ref01_data = helpers.to_map(projects_job_token_scope_ref01_data_raw[0][1])

        # UPDATE
        projects_job_token_scope_ref01_ent = client.ProjectsJobTokenScope(None)
        projects_job_token_scope_ref01_data_up0_up = {
        }

        projects_job_token_scope_ref01_resdata_up0 = helpers.to_map(runner.entity_data(projects_job_token_scope_ref01_ent.update(projects_job_token_scope_ref01_data_up0_up, None)))
        assert projects_job_token_scope_ref01_resdata_up0 is not None



def _projects_job_token_scope_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/projects_job_token_scope/ProjectsJobTokenScopeTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "GITLAB_TEST_PROJECTS_JOB_TOKEN_SCOPE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_PROJECTS_JOB_TOKEN_SCOPE_ENTID"))
