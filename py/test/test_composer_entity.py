...
class TestComposerEntity:

    def test_should_create_instance(self):
        testsdk = GitlabSDK.test(None, None)
        ent = testsdk.Composer(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _composer_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "composer." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GITLAB_TEST_COMPOSER_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        composer_ref01_ent = client.Composer(None)
        ...
        composer_ref01_data = helpers.to_map(composer_ref01_ent.create(composer_ref01_data, None))
        assert composer_ref01_data is not None



def _composer_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/composer/ComposerTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GITLAB_TEST_COMPOSER_ENTID"))
