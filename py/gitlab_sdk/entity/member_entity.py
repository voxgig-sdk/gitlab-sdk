...
class MemberEntity:
    ...
    def update(self, reqdata: MemberUpdateData, ctrl=None) -> Member:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Member:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Member().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
