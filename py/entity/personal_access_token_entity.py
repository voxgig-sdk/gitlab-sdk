...
class PersonalAccessTokenEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PersonalAccessTokenEntity(self._client, opts)
    ...
    def data_get(self) -> PersonalAccessToken:
    ...
    def match_get(self) -> PersonalAccessToken:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PersonalAccessToken:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PersonalAccessToken().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
