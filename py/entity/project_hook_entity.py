...
class ProjectHookEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProjectHookEntity(self._client, opts)
    ...
    def match_get(self) -> ProjectHook:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ProjectHook:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ProjectHook().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
