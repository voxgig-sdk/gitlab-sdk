...
class TerraformRegistryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TerraformRegistryEntity(self._client, opts)
    ...
    def data_get(self) -> TerraformRegistry:
    ...
    def match_get(self) -> TerraformRegistry:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TerraformRegistry:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TerraformRegistry().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: TerraformRegistryUpdateData, ctrl=None) -> TerraformRegistry:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
