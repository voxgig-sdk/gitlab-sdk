...
class SecureFileEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SecureFileEntity(self._client, opts)
    ...
    def match_get(self) -> SecureFile:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> SecureFile:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.SecureFile().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> SecureFile:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.SecureFile().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
