...
class HookEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return HookEntity(self._client, opts)
    ...
    def create(self, reqdata: HookCreateData, ctrl=None) -> Hook:
        utility = self._utility
    ...
    def update(self, reqdata: HookUpdateData, ctrl=None) -> Hook:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Hook:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Hook().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
