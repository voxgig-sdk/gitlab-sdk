...
class DebianDistributionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DebianDistributionEntity(self._client, opts)
    ...
    def data_get(self) -> DebianDistribution:
    ...
    def match_get(self) -> DebianDistribution:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DebianDistribution:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DebianDistribution().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
