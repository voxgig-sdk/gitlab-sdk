...
class ContainerRegistryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ContainerRegistryEntity(self._client, opts)
    ...
    def data_get(self) -> ContainerRegistry:
    ...
    def match_get(self) -> ContainerRegistry:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ContainerRegistry:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ContainerRegistry().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
