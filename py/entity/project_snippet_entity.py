...
class ProjectSnippetEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProjectSnippetEntity(self._client, opts)
    ...
    def data_get(self) -> ProjectSnippet:
    ...
    def match_get(self) -> ProjectSnippet:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ProjectSnippet:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ProjectSnippet().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
