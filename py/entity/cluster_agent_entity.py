...
class ClusterAgentEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ClusterAgentEntity(self._client, opts)
    ...
    def data_get(self) -> ClusterAgent:
    ...
    def match_get(self) -> ClusterAgent:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ClusterAgent:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ClusterAgent().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
