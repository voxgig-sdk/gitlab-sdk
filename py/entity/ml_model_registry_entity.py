...
class MlModelRegistryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MlModelRegistryEntity(self._client, opts)
    ...
    def data_get(self) -> MlModelRegistry:
    ...
    def match_get(self) -> MlModelRegistry:
    ...
    def load(self, reqmatch=None, ctrl=None) -> MlModelRegistry:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.MlModelRegistry().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: MlModelRegistryUpdateData, ctrl=None) -> MlModelRegistry:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
