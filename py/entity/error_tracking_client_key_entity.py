...
class ErrorTrackingClientKeyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ErrorTrackingClientKeyEntity(self._client, opts)
    ...
    def data_get(self) -> ErrorTrackingClientKey:
    ...
    def match_get(self) -> ErrorTrackingClientKey:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ErrorTrackingClientKey:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ErrorTrackingClientKey().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
