...
class AlertManagementEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AlertManagementEntity(self._client, opts)
    ...
    def data_get(self) -> AlertManagement:
    ...
    def match_get(self) -> AlertManagement:
    ...
    def create(self, reqdata: AlertManagementCreateData, ctrl=None) -> AlertManagement:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> AlertManagement:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.AlertManagement().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
