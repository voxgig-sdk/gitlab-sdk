...
class FreezePeriodEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return FreezePeriodEntity(self._client, opts)
    ...
    def data_get(self) -> FreezePeriod:
    ...
    def match_get(self) -> FreezePeriod:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> FreezePeriod:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.FreezePeriod().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
