...
class AccessRequestEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AccessRequestEntity(self._client, opts)
    ...
    def data_get(self) -> AccessRequest:
    ...
    def match_get(self) -> AccessRequest:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> AccessRequest:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.AccessRequest().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
