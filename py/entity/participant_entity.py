...
class ParticipantEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ParticipantEntity(self._client, opts)
    ...
    def match_get(self) -> Participant:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Participant]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Participant().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
