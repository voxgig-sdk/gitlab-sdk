...
class PackageFileEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PackageFileEntity(self._client, opts)
    ...
    def match_get(self) -> PackageFile:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PackageFile:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PackageFile().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PackageFile:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PackageFile().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
