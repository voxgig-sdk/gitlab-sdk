...
class DebianPackageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DebianPackageEntity(self._client, opts)
    ...
    def data_get(self) -> DebianPackage:
    ...
    def match_get(self) -> DebianPackage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DebianPackage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DebianPackage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: DebianPackageUpdateData, ctrl=None) -> DebianPackage:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
