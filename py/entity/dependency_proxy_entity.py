...
class DependencyProxyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DependencyProxyEntity(self._client, opts)
    ...
    def data_get(self) -> DependencyProxy:
    ...
    def match_get(self) -> DependencyProxy:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DependencyProxy:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DependencyProxy().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
