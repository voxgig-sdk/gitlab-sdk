...
class RubygemEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RubygemEntity(self._client, opts)
    ...
    def match_get(self) -> Rubygem:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> Rubygem:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Rubygem().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
