...
class HelmPackageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return HelmPackageEntity(self._client, opts)
    ...
    def match_get(self) -> HelmPackage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> HelmPackage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.HelmPackage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: HelmPackageCreateData, ctrl=None) -> HelmPackage:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
