...
class ReleaseLinkEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ReleaseLinkEntity(self._client, opts)
    ...
    def match_get(self) -> ReleaseLink:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ReleaseLink:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ReleaseLink().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
