...
class ApiEntitiesNpmPackageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ApiEntitiesNpmPackageEntity(self._client, opts)
    ...
    def data_get(self) -> ApiEntitiesNpmPackage:
    ...
    def match_get(self) -> ApiEntitiesNpmPackage:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> ApiEntitiesNpmPackage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ApiEntitiesNpmPackage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
