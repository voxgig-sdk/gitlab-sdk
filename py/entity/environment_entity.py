...
class EnvironmentEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EnvironmentEntity(self._client, opts)
    ...
    def match_get(self) -> Environment:
    ...
    def create(self, reqdata: EnvironmentCreateData, ctrl=None) -> Environment:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Environment:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Environment().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
