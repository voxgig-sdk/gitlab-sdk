...
class GroupAvatarEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GroupAvatarEntity(self._client, opts)
    ...
    def match_get(self) -> GroupAvatar:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GroupAvatar:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GroupAvatar().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
