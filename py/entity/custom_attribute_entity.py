...
class CustomAttributeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CustomAttributeEntity(self._client, opts)
    ...
    def data_get(self) -> CustomAttribute:
    ...
    def match_get(self) -> CustomAttribute:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> CustomAttribute:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CustomAttribute().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
