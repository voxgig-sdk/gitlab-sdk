...
class MavenPackageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MavenPackageEntity(self._client, opts)
    ...
    def data_get(self) -> MavenPackage:
    ...
    def match_get(self) -> MavenPackage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> MavenPackage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.MavenPackage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: MavenPackageUpdateData, ctrl=None) -> MavenPackage:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
