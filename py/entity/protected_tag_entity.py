...
class ProtectedTagEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProtectedTagEntity(self._client, opts)
    ...
    def data_get(self) -> ProtectedTag:
    ...
    def match_get(self) -> ProtectedTag:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ProtectedTag:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ProtectedTag().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
