...
func TestPageEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_PAGE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func pageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"page01", "page02", "page03", "project01", "project02", "project03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_PAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITLAB_TEST_PAGE_ENTID"])
