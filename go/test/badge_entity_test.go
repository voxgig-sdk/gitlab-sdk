...
func TestBadgeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		badgeRef01MatchRm0 := map[string]any{
			"id": badgeRef01Data["id"],
		}
		_, err := badgeRef01Ent.Remove(badgeRef01MatchRm0, nil)
		if err != nil {
...
func badgeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"badge01", "badge02", "badge03", "group01", "group02", "group03", "project01", "project02", "project03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_BADGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITLAB_TEST_BADGE_ENTID"])
