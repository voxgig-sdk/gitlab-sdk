...
func TestApiEntitiesTagEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_tag": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesTag(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesTag(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesTagRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_tag"}), "api_entities_tag_ref01"))
		...
		apiEntitiesTagRef01Data = core.ToMapAny(entityData(apiEntitiesTagRef01DataResult))
