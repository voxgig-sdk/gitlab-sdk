...
func TestTagEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_TAG_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		tagRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.tag")))
...
func tagBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"tag01", "tag02", "tag03", "project01", "project02", "project03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_TAG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITLAB_TEST_TAG_ENTID"])
