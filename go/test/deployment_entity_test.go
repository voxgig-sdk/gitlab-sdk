...
func TestDeploymentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		deploymentRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.deployment")))
		var deploymentRef01Data map[string]any
		if len(deploymentRef01DataRaw) > 0 {
			deploymentRef01Data = core.ToMapAny(deploymentRef01DataRaw[0][1])
		}
...
func deploymentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"deployment01", "deployment02", "deployment03", "project01", "project02", "project03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_DEPLOYMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITLAB_TEST_DEPLOYMENT_ENTID"])
