...
func TestApiEntitiesPackagesConanPackageManifestDirect(t *testing.T) {
	t.Run("direct-load-api_entities_packages_conan_package_manifest", func(t *testing.T) {
		setup := api_entities_packages_conan_package_manifestDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-api_entities_packages_conan_package_manifest", _mode); _shouldSkip {
			if _reason == "" {
...
type api_entities_packages_conan_package_manifestDirectSetupResult struct {
...
func api_entities_packages_conan_package_manifestDirectSetup(mockres any) *api_entities_packages_conan_package_manifestDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_PACKAGE_MANIFEST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_PACKAGE_MANIFEST_ENTID"]; ok {
		...
		return &api_entities_packages_conan_package_manifestDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &api_entities_packages_conan_package_manifestDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
