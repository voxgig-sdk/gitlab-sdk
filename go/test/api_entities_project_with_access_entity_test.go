...
func TestApiEntitiesProjectWithAccessEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.ApiEntitiesProjectWithAccess(nil)
		if ent == nil {
			t.Fatal("expected non-nil ApiEntitiesProjectWithAccessEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := api_entities_project_with_accessBasicSetup(nil)
		...
		apiEntitiesProjectWithAccessRef01DataDt0Loaded, err := apiEntitiesProjectWithAccessRef01Ent.Load(apiEntitiesProjectWithAccessRef01MatchDt0, nil)
		if err != nil {
		...
		if apiEntitiesProjectWithAccessRef01DataDt0LoadResult["id"] != apiEntitiesProjectWithAccessRef01Data["id"] {
...
func api_entities_project_with_accessBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "api_entities_project_with_access", "ApiEntitiesProjectWithAccessTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read api_entities_project_with_access test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse api_entities_project_with_access test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_API_ENTITIES_PROJECT_WITH_ACCESS_ENTID")
