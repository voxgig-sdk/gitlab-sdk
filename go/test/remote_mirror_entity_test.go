...
func TestRemoteMirrorEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.RemoteMirror(nil)
		if ent == nil {
			t.Fatal("expected non-nil RemoteMirrorEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := remote_mirrorBasicSetup(nil)
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_REMOTE_MIRROR_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		remoteMirrorRef01Data = core.ToMapAny(remoteMirrorRef01DataResult)
...
func remote_mirrorBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "remote_mirror", "RemoteMirrorTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read remote_mirror test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse remote_mirror test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_REMOTE_MIRROR_ENTID")
