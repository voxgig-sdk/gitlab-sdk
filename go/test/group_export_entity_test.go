...
func TestGroupExportEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.GroupExport(nil)
		if ent == nil {
			t.Fatal("expected non-nil GroupExportEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := group_exportBasicSetup(nil)
		...
		for _, _op := range []string{"create", "load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "group_export." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_GROUP_EXPORT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func group_exportBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "group_export", "GroupExportTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read group_export test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse group_export test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_GROUP_EXPORT_ENTID")
