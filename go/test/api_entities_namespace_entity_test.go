...
func TestApiEntitiesNamespaceEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_namespace": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesNamespace(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesNamespace(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesNamespaceRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.api_entities_namespace")))
