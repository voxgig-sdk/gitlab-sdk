...
func TestConanPackageDirect(t *testing.T) {
	t.Run("direct-load-conan_package", func(t *testing.T) {
		setup := conan_packageDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-conan_package", _mode); _shouldSkip {
			if _reason == "" {
...
type conan_packageDirectSetupResult struct {
...
func conan_packageDirectSetup(mockres any) *conan_packageDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITLAB_TEST_CONAN_PACKAGE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITLAB_TEST_CONAN_PACKAGE_ENTID"]; ok {
		...
		return &conan_packageDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &conan_packageDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
