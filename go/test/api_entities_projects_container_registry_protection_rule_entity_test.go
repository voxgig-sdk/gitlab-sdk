	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_projects_container_registry_protection_rule": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesProjectsContainerRegistryProtectionRule(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesProjectsContainerRegistryProtectionRule(nil).Stream("list", nil, nil) {
