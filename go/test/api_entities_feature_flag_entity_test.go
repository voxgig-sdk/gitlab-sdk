...
func TestApiEntitiesFeatureFlagEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_feature_flag": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesFeatureFlag(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesFeatureFlag(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesFeatureFlagRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_feature_flag"}), "api_entities_feature_flag_ref01"))
