...
func TestApiEntitiesProjectHookEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_project_hook": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesProjectHook(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesProjectHook(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesProjectHookRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_project_hook"}), "api_entities_project_hook_ref01"))
		...
		apiEntitiesProjectHookRef01Data = core.ToMapAny(entityData(apiEntitiesProjectHookRef01DataResult))
		...
		apiEntitiesProjectHookRef01ResdataUp0 := core.ToMapAny(entityData(apiEntitiesProjectHookRef01ResdataUp0Result))
		...
		apiEntitiesProjectHookRef01DataDt0LoadResult := core.ToMapAny(entityData(apiEntitiesProjectHookRef01DataDt0Loaded))
