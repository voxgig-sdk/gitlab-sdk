...
func TestApiEntitiesPackagesConanPackageRevisionDirect(t *testing.T) {
	t.Run("direct-list-api_entities_packages_conan_package_revision", func(t *testing.T) {
		setup := api_entities_packages_conan_package_revisionDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-api_entities_packages_conan_package_revision", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"conan01", "conan_package_reference01", "package_channel01", "package_username01", "package_version01", "project01", "revision01"} {
...
type api_entities_packages_conan_package_revisionDirectSetupResult struct {
...
func api_entities_packages_conan_package_revisionDirectSetup(mockres any) *api_entities_packages_conan_package_revisionDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_PACKAGE_REVISION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_PACKAGE_REVISION_ENTID"]; ok {
		...
		return &api_entities_packages_conan_package_revisionDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &api_entities_packages_conan_package_revisionDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
