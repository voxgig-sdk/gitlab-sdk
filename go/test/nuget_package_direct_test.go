...
func TestNugetPackageDirect(t *testing.T) {
	t.Run("direct-list-nuget_package", func(t *testing.T) {
		setup := nuget_packageDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-nuget_package", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-nuget_package", func(t *testing.T) {
		setup := nuget_packageDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-nuget_package", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type nuget_packageDirectSetupResult struct {
...
func nuget_packageDirectSetup(mockres any) *nuget_packageDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITLAB_TEST_NUGET_PACKAGE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITLAB_TEST_NUGET_PACKAGE_ENTID"]; ok {
		...
		return &nuget_packageDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &nuget_packageDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
