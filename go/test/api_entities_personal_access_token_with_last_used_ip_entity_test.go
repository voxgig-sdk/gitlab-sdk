...
func TestApiEntitiesPersonalAccessTokenWithLastUsedIpEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_personal_access_token_with_last_used_ip": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesPersonalAccessTokenWithLastUsedIp(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesPersonalAccessTokenWithLastUsedIp(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesPersonalAccessTokenWithLastUsedIpRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.api_entities_personal_access_token_with_last_used_ip")))
		...
		apiEntitiesPersonalAccessTokenWithLastUsedIpRef01DataDt0LoadResult := core.ToMapAny(entityData(apiEntitiesPersonalAccessTokenWithLastUsedIpRef01DataDt0Loaded))
