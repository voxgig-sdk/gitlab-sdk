...
func TestApiEntitiesHookEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_hook": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesHook(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesHook(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesHookRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_hook"}), "api_entities_hook_ref01"))

		apiEntitiesHookRef01DataResult, err := apiEntitiesHookRef01Ent.Create(apiEntitiesHookRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		apiEntitiesHookRef01Data = core.ToMapAny(entityData(apiEntitiesHookRef01DataResult))
		...
		apiEntitiesHookRef01ResdataUp0 := core.ToMapAny(entityData(apiEntitiesHookRef01ResdataUp0Result))
		...
		apiEntitiesHookRef01DataDt0LoadResult := core.ToMapAny(entityData(apiEntitiesHookRef01DataDt0Loaded))
