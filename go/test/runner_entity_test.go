...
func TestRunnerEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.Runner(nil)
		if ent == nil {
			t.Fatal("expected non-nil RunnerEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := runnerBasicSetup(nil)
		...
		for _, _op := range []string{"create", "remove"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "runner." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_RUNNER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func runnerBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "runner", "RunnerTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read runner test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse runner test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_RUNNER_ENTID")
