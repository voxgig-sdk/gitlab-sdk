...
func TestApiEntitiesInvitationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_invitation": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesInvitation(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesInvitation(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesInvitationRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_invitation"}), "api_entities_invitation_ref01"))
		...
		apiEntitiesInvitationRef01Data = core.ToMapAny(entityData(apiEntitiesInvitationRef01DataResult))
