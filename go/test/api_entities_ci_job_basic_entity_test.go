...
func TestApiEntitiesCiJobBasicEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_ci_job_basic": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesCiJobBasic(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesCiJobBasic(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesCiJobBasicRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_ci_job_basic"}), "api_entities_ci_job_basic_ref01"))
		...
		apiEntitiesCiJobBasicRef01Data = core.ToMapAny(entityData(apiEntitiesCiJobBasicRef01DataResult))
