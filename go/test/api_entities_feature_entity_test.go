...
func TestApiEntitiesFeatureEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_feature": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesFeature(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesFeature(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesFeatureRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_feature"}), "api_entities_feature_ref01"))
		...
		apiEntitiesFeatureRef01Data = core.ToMapAny(entityData(apiEntitiesFeatureRef01DataResult))
