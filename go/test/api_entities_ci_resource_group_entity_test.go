...
func TestApiEntitiesCiResourceGroupEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_ci_resource_group": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesCiResourceGroup(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesCiResourceGroup(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesCiResourceGroupRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.api_entities_ci_resource_group")))
		...
		apiEntitiesCiResourceGroupRef01ResdataUp0 := core.ToMapAny(entityData(apiEntitiesCiResourceGroupRef01ResdataUp0Result))
		...
		apiEntitiesCiResourceGroupRef01DataDt0LoadResult := core.ToMapAny(entityData(apiEntitiesCiResourceGroupRef01DataDt0Loaded))
