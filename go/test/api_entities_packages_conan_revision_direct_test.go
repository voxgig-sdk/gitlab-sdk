...
func TestApiEntitiesPackagesConanRevisionDirect(t *testing.T) {
	t.Run("direct-load-api_entities_packages_conan_revision", func(t *testing.T) {
		setup := api_entities_packages_conan_revisionDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-api_entities_packages_conan_revision", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"conan_id01", "conan_package_reference01", "package_channel01", "package_username01", "package_version01", "project_id01", "revision_id01"} {
...
type api_entities_packages_conan_revisionDirectSetupResult struct {
...
func api_entities_packages_conan_revisionDirectSetup(mockres any) *api_entities_packages_conan_revisionDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_REVISION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_REVISION_ENTID"]; ok {
		...
		return &api_entities_packages_conan_revisionDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &api_entities_packages_conan_revisionDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
