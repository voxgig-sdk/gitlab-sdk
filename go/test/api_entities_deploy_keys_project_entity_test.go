...
func TestApiEntitiesDeployKeysProjectEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		seed := map[string]any{
			"entity": map[string]any{
				"api_entities_deploy_keys_project": map[string]any{
		...
		var seen []any
		for item := range base.ApiEntitiesDeployKeysProject(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApiEntitiesDeployKeysProject(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		apiEntitiesDeployKeysProjectRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "api_entities_deploy_keys_project"}), "api_entities_deploy_keys_project_ref01"))
