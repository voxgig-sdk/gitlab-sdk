...
func TestImportEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		importRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "import"}, setup.data), "import_ref01"))

		importRef01DataResult, err := importRef01Ent.Create(importRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		importRef01Data = core.ToMapAny(importRef01DataResult)
...
func importBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_IMPORT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITLAB_TEST_IMPORT_ENTID"])
