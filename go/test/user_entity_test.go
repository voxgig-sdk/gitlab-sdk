...
func TestUserEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_USER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"user01", "user02", "user03", "project01", "project02", "project03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GITLAB_TEST_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITLAB_TEST_USER_ENTID"])
