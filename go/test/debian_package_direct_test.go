...
func TestDebianPackageDirect(t *testing.T) {
	t.Run("direct-load-debian_package", func(t *testing.T) {
		setup := debian_packageDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-debian_package", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"distribution01", "file_name01", "id01", "letter01", "package_name01", "package_version01", "project_id01"} {
...
type debian_packageDirectSetupResult struct {
...
func debian_packageDirectSetup(mockres any) *debian_packageDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GITLAB_TEST_DEBIAN_PACKAGE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GITLAB_TEST_DEBIAN_PACKAGE_ENTID"]; ok {
		...
		return &debian_packageDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &debian_packageDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
