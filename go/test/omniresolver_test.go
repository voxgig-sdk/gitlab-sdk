...
import (
	...
	omni "github.com/voxgig-sdk/gitlab-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GitlabError); is {
				out := map[string]any{
					"name":    "GitlabError",
...
func omniCtx(arg any, client *sdk.GitlabSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GitlabSDK); is {
			client = live
