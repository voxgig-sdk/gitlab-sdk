...
func NewReleaseLinkEntity(client *core.GitlabSDK, entopts map[string]any) *ReleaseLinkEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *ReleaseLinkEntity) GetName() string { return e.name }
...
func (e *ReleaseLinkEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *ReleaseLinkEntity) Deleted() bool {
	return e.deleted
}


func (e *ReleaseLinkEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewReleaseLinkEntity(e.client, opts)
}

func (e *ReleaseLinkEntity) Data(args ...any) any {
...
func (e *ReleaseLinkEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an ReleaseLink; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *ReleaseLinkEntity) DataTyped(data ...ReleaseLink) ReleaseLink {
	if len(data) > 0 {
		return typedFrom[ReleaseLink](e.Data(asMap(data[0])))
	}
	return typedFrom[ReleaseLink](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through ReleaseLink (all fields
// optional at the wire level).
func (e *ReleaseLinkEntity) MatchTyped(match ...ReleaseLink) ReleaseLink {
	if len(match) > 0 {
		return typedFrom[ReleaseLink](e.Match(asMap(match[0])))
	}
	return typedFrom[ReleaseLink](e.Match())
}
...
func (e *ReleaseLinkEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *ReleaseLinkEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *ReleaseLinkEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *ReleaseLinkEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *ReleaseLinkEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}



func (e *ReleaseLinkEntity) Remove(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// ReleaseLinkRemoveMatch and returns an ReleaseLink. It delegates to the untyped
// Remove (identical runtime) and converts at the typed boundary.
func (e *ReleaseLinkEntity) RemoveTyped(reqmatch ReleaseLinkRemoveMatch, ctrl map[string]any) (ReleaseLink, error) {
	res, err := e.Remove(asMap(reqmatch), ctrl)
	if err != nil {
		return ReleaseLink{}, err
	}
	return typedFrom[ReleaseLink](res), nil
}



func (e *ReleaseLinkEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
