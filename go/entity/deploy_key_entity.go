...
func NewDeployKeyEntity(client *core.GitlabSDK, entopts map[string]any) *DeployKeyEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *DeployKeyEntity) GetName() string { return e.name }
...
func (e *DeployKeyEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *DeployKeyEntity) Deleted() bool {
	return e.deleted
}


func (e *DeployKeyEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewDeployKeyEntity(e.client, opts)
}

func (e *DeployKeyEntity) Data(args ...any) any {
...
func (e *DeployKeyEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an DeployKey; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *DeployKeyEntity) DataTyped(data ...DeployKey) DeployKey {
	if len(data) > 0 {
		return typedFrom[DeployKey](e.Data(asMap(data[0])))
	}
	return typedFrom[DeployKey](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through DeployKey (all fields
// optional at the wire level).
func (e *DeployKeyEntity) MatchTyped(match ...DeployKey) DeployKey {
	if len(match) > 0 {
		return typedFrom[DeployKey](e.Match(asMap(match[0])))
	}
	return typedFrom[DeployKey](e.Match())
}
...
func (e *DeployKeyEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *DeployKeyEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *DeployKeyEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *DeployKeyEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *DeployKeyEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}



func (e *DeployKeyEntity) Remove(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// DeployKeyRemoveMatch and returns an DeployKey. It delegates to the untyped
// Remove (identical runtime) and converts at the typed boundary.
func (e *DeployKeyEntity) RemoveTyped(reqmatch DeployKeyRemoveMatch, ctrl map[string]any) (DeployKey, error) {
	res, err := e.Remove(asMap(reqmatch), ctrl)
	if err != nil {
		return DeployKey{}, err
	}
	return typedFrom[DeployKey](res), nil
}



func (e *DeployKeyEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
