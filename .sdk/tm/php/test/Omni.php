...
//    only - a typed GitlabContext there would make every ctx
...
final class GitlabOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GitlabContext($clean, null);
    ...
    public static function err_from_map($m): ?GitlabError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GitlabError(is_string($code) ? $code : '', $msg);
    }
