...
boru-driven command-line client **and** interactive REPL for the Gitlab
...
./gitlab-cli load 1 api_entities_project_with_access            # {id:1} shorthand
./gitlab-cli load '{id:1}' api_entities_project_with_access       # explicit match map

# 5. Override the API base URL for a single call
GITLAB_BASE=https://api.example.com ./gitlab-cli load 1 api_entities_project_with_access

# 6. No arguments -> interactive REPL
./gitlab-cli
gitlab> load 1 api_entities_project_with_access
gitlab> /quit
```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/gitlab-cli load 1 api_entities_project_with_access
   ```
...
```sh
./gitlab-cli load 1 api_entities_project_with_access          # scalar shorthand for {id:1}
./gitlab-cli load '{id:1}' api_entities_project_with_access     # explicit match map
```
...
./gitlab-cli load 1 api_entities_project_with_access
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `api_entities_project_with_access`).
...
api_entities_project_with_access project
