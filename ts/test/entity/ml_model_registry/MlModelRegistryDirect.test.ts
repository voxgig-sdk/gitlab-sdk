...
describe('MlModelRegistryDirect', async () => {
  ...
  test('direct-load-ml_model_registry', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ml_model_registry', setup.live)) return
    if (skipIfMissingIds(t, setup, ["ml_model_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_ML_MODEL_REGISTRY_ENTID']
