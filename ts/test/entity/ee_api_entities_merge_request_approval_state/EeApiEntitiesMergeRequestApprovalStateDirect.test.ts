...
describe('EeApiEntitiesMergeRequestApprovalStateDirect', async () => {
  ...
  test('direct-list-ee_api_entities_merge_request_approval_state', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ee_api_entities_merge_request_approval_state', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_EE_API_ENTITIES_MERGE_REQUEST_APPROVAL_STATE_ENTID']
