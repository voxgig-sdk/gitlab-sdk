...
describe('PypiPackageDirect', async () => {
  ...
  test('direct-load-pypi_package', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-pypi_package', setup.live)) return
    if (skipIfMissingIds(t, setup, ["group_id01","sha25601"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_PYPI_PACKAGE_ENTID']
