...
describe('ApiEntitiesBulkImportsEntityFailureDirect', async () => {
  ...
  test('direct-load-api_entities_bulk_imports_entity_failure', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_bulk_imports_entity_failure', setup.live)) return
    if (skipIfMissingIds(t, setup, ["bulk_import_id01","entity_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_BULK_IMPORTS_ENTITY_FAILURE_ENTID']
