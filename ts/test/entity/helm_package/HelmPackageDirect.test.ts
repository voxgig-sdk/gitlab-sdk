...
describe('HelmPackageDirect', async () => {
  ...
  test('direct-load-helm_package', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-helm_package', setup.live)) return
    if (skipIfMissingIds(t, setup, ["helm_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_HELM_PACKAGE_ENTID']
