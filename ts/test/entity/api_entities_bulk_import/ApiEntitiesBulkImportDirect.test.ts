...
describe('ApiEntitiesBulkImportDirect', async () => {
  ...
  test('direct-load-api_entities_bulk_import', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_bulk_import', setup.live)) return
    if (skipIfMissingIds(t, setup, ["bulk_import01","entity01"])) return
  ...
  test('direct-list-api_entities_bulk_import', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_bulk_import', setup.live)) return
    if (skipIfMissingIds(t, setup, ["bulk_import01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_BULK_IMPORT_ENTID']
