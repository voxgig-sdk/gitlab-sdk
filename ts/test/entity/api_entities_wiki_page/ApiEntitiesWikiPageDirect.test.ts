...
describe('ApiEntitiesWikiPageDirect', async () => {
  ...
  test('direct-load-api_entities_wiki_page', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_wiki_page', setup.live)) return
    if (skipIfMissingIds(t, setup, ["group_id01","slug01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_WIKI_PAGE_ENTID']
