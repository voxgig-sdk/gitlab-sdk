...
describe('CustomAttributeDirect', async () => {
  ...
  test('direct-load-custom_attribute', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-custom_attribute', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_CUSTOM_ATTRIBUTE_ENTID']
