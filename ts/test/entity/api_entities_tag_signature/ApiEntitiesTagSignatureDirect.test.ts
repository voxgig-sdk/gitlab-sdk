...
describe('ApiEntitiesTagSignatureDirect', async () => {
  ...
  test('direct-load-api_entities_tag_signature', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_tag_signature', setup.live)) return
    if (skipIfMissingIds(t, setup, ["project_id01","tag_name01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_TAG_SIGNATURE_ENTID']
