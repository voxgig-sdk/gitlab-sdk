...
describe('ApiEntitiesNugetSearchResultEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_nuget_search_result.' + op, live)) return
    }
    ...
    const api_entities_nuget_search_result_ref01_list = await api_entities_nuget_search_result_ref01_ent.list(api_entities_nuget_search_result_ref01_match)
