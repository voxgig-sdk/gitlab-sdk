...
describe('ApiEntitiesMergeDirect', async () => {
  ...
  test('direct-load-api_entities_merge', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_merge', setup.live)) return
    if (skipIfMissingIds(t, setup, ["merge_request_iid01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_MERGE_ENTID']
