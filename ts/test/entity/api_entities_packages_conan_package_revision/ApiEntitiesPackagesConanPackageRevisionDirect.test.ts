...
describe('ApiEntitiesPackagesConanPackageRevisionDirect', async () => {
  ...
  test('direct-list-api_entities_packages_conan_package_revision', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_packages_conan_package_revision', setup.live)) return
    if (skipIfMissingIds(t, setup, ["conan01","conan_package_reference01","package_channel01","package_username01","package_version01","project01","revision01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_PACKAGE_REVISION_ENTID']
