...
describe('EeApiEntitiesAuditEventDirect', async () => {
  ...
  test('direct-load-ee_api_entities_audit_event', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ee_api_entities_audit_event', setup.live)) return
  ...
  test('direct-list-ee_api_entities_audit_event', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ee_api_entities_audit_event', setup.live)) return
    if (skipIfMissingIds(t, setup, ["group01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_EE_API_ENTITIES_AUDIT_EVENT_ENTID']
