...
describe('ApiEntitiesCiRunnerRegistrationDetailEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_ci_runner_registration_detail.' + op, live)) return
    }
    ...
    api_entities_ci_runner_registration_detail_ref01_data = await api_entities_ci_runner_registration_detail_ref01_ent.create(api_entities_ci_runner_registration_detail_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_CI_RUNNER_REGISTRATION_DETAIL_ENTID']
