...
describe('ApiEntitiesCommitSequenceDirect', async () => {
  ...
  test('direct-load-api_entities_commit_sequence', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_commit_sequence', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_COMMIT_SEQUENCE_ENTID']
