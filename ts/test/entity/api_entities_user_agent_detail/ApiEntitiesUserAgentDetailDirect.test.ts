...
describe('ApiEntitiesUserAgentDetailDirect', async () => {
  ...
  test('direct-load-api_entities_user_agent_detail', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_user_agent_detail', setup.live)) return
    if (skipIfMissingIds(t, setup, ["issue_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_USER_AGENT_DETAIL_ENTID']
