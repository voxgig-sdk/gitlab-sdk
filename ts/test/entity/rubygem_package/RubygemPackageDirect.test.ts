...
describe('RubygemPackageDirect', async () => {
  ...
  test('direct-load-rubygem_package', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-rubygem_package', setup.live)) return
    if (skipIfMissingIds(t, setup, ["file_name01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_RUBYGEM_PACKAGE_ENTID']
