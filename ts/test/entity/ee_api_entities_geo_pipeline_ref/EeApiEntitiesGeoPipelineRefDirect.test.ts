...
describe('EeApiEntitiesGeoPipelineRefDirect', async () => {
  ...
  test('direct-list-ee_api_entities_geo_pipeline_ref', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ee_api_entities_geo_pipeline_ref', setup.live)) return
    if (skipIfMissingIds(t, setup, ["gl_repository01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_EE_API_ENTITIES_GEO_PIPELINE_REF_ENTID']
