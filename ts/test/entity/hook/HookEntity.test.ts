...
describe('HookEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'update', 'remove']) {
      if (maybeSkipControl(t, 'entityOp', 'hook.' + op, live)) return
    }
    ...
    hook_ref01_data = await hook_ref01_ent.create(hook_ref01_data)
    ...
    const hook_ref01_resdata_up0 = await hook_ref01_ent.update(hook_ref01_data_up0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_HOOK_ENTID']
