...
describe('TerraformRegistryDirect', async () => {
  ...
  test('direct-load-terraform_registry', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-terraform_registry', setup.live)) return
    if (skipIfMissingIds(t, setup, ["module_id01","module_system01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_TERRAFORM_REGISTRY_ENTID']
