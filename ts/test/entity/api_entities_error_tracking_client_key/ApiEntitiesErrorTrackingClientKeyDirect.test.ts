...
describe('ApiEntitiesErrorTrackingClientKeyDirect', async () => {
  ...
  test('direct-list-api_entities_error_tracking_client_key', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_error_tracking_client_key', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_ERROR_TRACKING_CLIENT_KEY_ENTID']
