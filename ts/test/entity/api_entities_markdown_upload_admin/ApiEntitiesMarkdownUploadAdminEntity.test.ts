...
describe('ApiEntitiesMarkdownUploadAdminEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_markdown_upload_admin.' + op, live)) return
    }
    ...
    const api_entities_markdown_upload_admin_ref01_list = await api_entities_markdown_upload_admin_ref01_ent.list(api_entities_markdown_upload_admin_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_MARKDOWN_UPLOAD_ADMIN_ENTID']
