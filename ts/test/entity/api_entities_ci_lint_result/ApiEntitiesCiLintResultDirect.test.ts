...
describe('ApiEntitiesCiLintResultDirect', async () => {
  ...
  test('direct-list-api_entities_ci_lint_result', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_ci_lint_result', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_CI_LINT_RESULT_ENTID']
