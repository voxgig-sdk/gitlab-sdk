...
describe('ApiEntitiesNamespacesStorageLimitExclusionDirect', async () => {
  ...
  test('direct-load-api_entities_namespaces_storage_limit_exclusion', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_namespaces_storage_limit_exclusion', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_NAMESPACES_STORAGE_LIMIT_EXCLUSION_ENTID']
