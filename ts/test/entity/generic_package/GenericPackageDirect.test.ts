...
describe('GenericPackageDirect', async () => {
  ...
  test('direct-load-generic_package', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-generic_package', setup.live)) return
    if (skipIfMissingIds(t, setup, ["generic_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_GENERIC_PACKAGE_ENTID']
