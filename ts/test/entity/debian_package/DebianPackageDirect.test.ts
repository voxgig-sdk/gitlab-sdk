...
describe('DebianPackageDirect', async () => {
  ...
  test('direct-load-debian_package', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-debian_package', setup.live)) return
    if (skipIfMissingIds(t, setup, ["distribution01","file_name01","id01","letter01","package_name01","package_version01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_DEBIAN_PACKAGE_ENTID']
