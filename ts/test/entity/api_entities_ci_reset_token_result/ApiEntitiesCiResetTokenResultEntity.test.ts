...
describe('ApiEntitiesCiResetTokenResultEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_ci_reset_token_result.' + op, live)) return
    }
    ...
    api_entities_ci_reset_token_result_ref01_data = await api_entities_ci_reset_token_result_ref01_ent.create(api_entities_ci_reset_token_result_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_CI_RESET_TOKEN_RESULT_ENTID']
