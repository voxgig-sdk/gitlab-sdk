...
describe('ApiEntitiesPackagesConanPackageManifestDirect', async () => {
  ...
  test('direct-load-api_entities_packages_conan_package_manifest', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_packages_conan_package_manifest', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_PACKAGE_MANIFEST_ENTID']
