...
describe('ApiEntitiesProjectsTopicDirect', async () => {
  ...
  test('direct-load-api_entities_projects_topic', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_projects_topic', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_PROJECTS_TOPIC_ENTID']
