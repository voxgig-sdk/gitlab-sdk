...
describe('ApiEntitiesMergeRequestChangeEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_merge_request_change.' + op, live)) return
    }
    ...
    const api_entities_merge_request_change_ref01_data_dt0 = await api_entities_merge_request_change_ref01_ent.load(api_entities_merge_request_change_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_MERGE_REQUEST_CHANGE_ENTID']
