...
describe('ApiEntitiesGitlabSubscriptionDirect', async () => {
  ...
  test('direct-load-api_entities_gitlab_subscription', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_gitlab_subscription', setup.live)) return
    if (skipIfMissingIds(t, setup, ["namespace_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_GITLAB_SUBSCRIPTION_ENTID']
