...
describe('SnippetDirect', async () => {
  ...
  test('direct-load-snippet', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-snippet', setup.live)) return
    if (skipIfMissingIds(t, setup, ["file_id01","file_path01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_SNIPPET_ENTID']
