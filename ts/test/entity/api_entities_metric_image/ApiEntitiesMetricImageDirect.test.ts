...
describe('ApiEntitiesMetricImageDirect', async () => {
  ...
  test('direct-list-api_entities_metric_image', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_metric_image', setup.live)) return
    if (skipIfMissingIds(t, setup, ["alert_management_alert01","project01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_METRIC_IMAGE_ENTID']
