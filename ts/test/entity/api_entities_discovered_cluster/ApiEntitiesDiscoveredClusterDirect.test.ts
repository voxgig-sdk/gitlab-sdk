...
describe('ApiEntitiesDiscoveredClusterDirect', async () => {
  ...
  test('direct-load-api_entities_discovered_cluster', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_discovered_cluster', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_DISCOVERED_CLUSTER_ENTID']
