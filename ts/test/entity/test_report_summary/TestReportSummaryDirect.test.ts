...
describe('TestReportSummaryDirect', async () => {
  ...
  test('direct-load-test_report_summary', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-test_report_summary', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_TEST_REPORT_SUMMARY_ENTID']
