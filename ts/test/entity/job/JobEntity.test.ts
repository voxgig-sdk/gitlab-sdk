...
describe('JobEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'job.' + op, live)) return
    }
    ...
    job_ref01_data = await job_ref01_ent.create(job_ref01_data)
    ...
    const job_ref01_resdata_up0 = await job_ref01_ent.update(job_ref01_data_up0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_JOB_ENTID']
