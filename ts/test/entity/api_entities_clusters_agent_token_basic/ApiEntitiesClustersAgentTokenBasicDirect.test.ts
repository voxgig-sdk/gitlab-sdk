...
describe('ApiEntitiesClustersAgentTokenBasicDirect', async () => {
  ...
  test('direct-load-api_entities_clusters_agent_token_basic', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_clusters_agent_token_basic', setup.live)) return
    if (skipIfMissingIds(t, setup, ["cluster_agent_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_CLUSTERS_AGENT_TOKEN_BASIC_ENTID']
