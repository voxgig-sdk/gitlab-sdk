...
describe('ApiEntitiesDictionaryTableDirect', async () => {
  ...
  test('direct-load-api_entities_dictionary_table', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_dictionary_table', setup.live)) return
    if (skipIfMissingIds(t, setup, ["databas_id01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_DICTIONARY_TABLE_ENTID']
