...
describe('EeApiEntitiesBillableMembershipDirect', async () => {
  ...
  test('direct-load-ee_api_entities_billable_membership', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ee_api_entities_billable_membership', setup.live)) return
    if (skipIfMissingIds(t, setup, ["billable_member_id01","group_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_EE_API_ENTITIES_BILLABLE_MEMBERSHIP_ENTID']
