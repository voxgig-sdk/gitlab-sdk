...
describe('ApiEntitiesGoModuleVersionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_go_module_version.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['api_entities_go_module_version01','api_entities_go_module_version02','api_entities_go_module_version03','project01','project02','project03','@v01','@v02','@v03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
