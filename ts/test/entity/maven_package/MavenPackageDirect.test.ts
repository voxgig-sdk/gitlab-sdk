...
describe('MavenPackageDirect', async () => {
  ...
  test('direct-load-maven_package', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-maven_package', setup.live)) return
    if (skipIfMissingIds(t, setup, ["file_name01","group_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_MAVEN_PACKAGE_ENTID']
