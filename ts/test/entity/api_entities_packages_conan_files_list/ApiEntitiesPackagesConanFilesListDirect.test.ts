...
describe('ApiEntitiesPackagesConanFilesListDirect', async () => {
  ...
  test('direct-load-api_entities_packages_conan_files_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_packages_conan_files_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["conan_id01","package_channel01","package_id01","package_revision01","package_username01","package_version01","project_id01","revision_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_FILES_LIST_ENTID']
