...
describe('ApiEntitiesMergeRequestApprovalDirect', async () => {
  ...
  test('direct-load-api_entities_merge_request_approval', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_merge_request_approval', setup.live)) return
    if (skipIfMissingIds(t, setup, ["merge_request_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_MERGE_REQUEST_APPROVAL_ENTID']
