...
describe('MergeRequestDirect', async () => {
  ...
  test('direct-load-merge_request', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-merge_request', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_MERGE_REQUEST_ENTID']
