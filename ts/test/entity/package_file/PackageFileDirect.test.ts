...
describe('PackageFileDirect', async () => {
  ...
  test('direct-load-package_file', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-package_file', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","package_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_PACKAGE_FILE_ENTID']
