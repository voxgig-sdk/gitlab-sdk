...
describe('ApiEntitiesTagEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_tag.' + op, live)) return
    }
    ...
    api_entities_tag_ref01_data = await api_entities_tag_ref01_ent.create(api_entities_tag_ref01_data)
    assert(null != api_entities_tag_ref01_data)
    ...
    const api_entities_tag_ref01_list = await api_entities_tag_ref01_ent.list(api_entities_tag_ref01_match)
