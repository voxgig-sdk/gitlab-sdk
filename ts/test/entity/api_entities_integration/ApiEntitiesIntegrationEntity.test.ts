...
describe('ApiEntitiesIntegrationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_integration.' + op, live)) return
    }
    ...
    const api_entities_integration_ref01_data_dt0 = await api_entities_integration_ref01_ent.load(api_entities_integration_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['api_entities_integration01','api_entities_integration02','api_entities_integration03','group01','group02','group03','project01','project02','project03','project01','project02','project03','service01','service02','service03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_INTEGRATION_ENTID']
