...
describe('ApiEntitiesDeployKeysProjectDirect', async () => {
  ...
  test('direct-load-api_entities_deploy_keys_project', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_deploy_keys_project', setup.live)) return
    if (skipIfMissingIds(t, setup, ["key01","project01"])) return
  ...
  test('direct-list-api_entities_deploy_keys_project', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_deploy_keys_project', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_DEPLOY_KEYS_PROJECT_ENTID']
