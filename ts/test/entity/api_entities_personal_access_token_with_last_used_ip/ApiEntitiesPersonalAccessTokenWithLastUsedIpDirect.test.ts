...
describe('ApiEntitiesPersonalAccessTokenWithLastUsedIpDirect', async () => {
  ...
  test('direct-load-api_entities_personal_access_token_with_last_used_ip', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_personal_access_token_with_last_used_ip', setup.live)) return
  ...
  test('direct-list-api_entities_personal_access_token_with_last_used_ip', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_personal_access_token_with_last_used_ip', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_PERSONAL_ACCESS_TOKEN_WITH_LAST_USED_IP_ENTID']
