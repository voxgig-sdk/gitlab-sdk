...
describe('ApiEntitiesPackageEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_package.' + op, live)) return
    }
    ...
    let api_entities_package_ref01_data = Object.values(setup.data.existing.api_entities_package)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/api_entities_package/ApiEntitiesPackageTestData.json')
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_PACKAGE_ENTID']
