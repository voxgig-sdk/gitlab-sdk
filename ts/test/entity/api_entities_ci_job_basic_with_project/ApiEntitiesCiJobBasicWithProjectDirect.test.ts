...
describe('ApiEntitiesCiJobBasicWithProjectDirect', async () => {
  ...
  test('direct-load-api_entities_ci_job_basic_with_project', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_ci_job_basic_with_project', setup.live)) return
    if (skipIfMissingIds(t, setup, ["runner_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_CI_JOB_BASIC_WITH_PROJECT_ENTID']
