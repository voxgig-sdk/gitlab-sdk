...
describe('ApiEntitiesCommitSignatureDirect', async () => {
  ...
  test('direct-load-api_entities_commit_signature', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_commit_signature', setup.live)) return
    if (skipIfMissingIds(t, setup, ["project_id01","sha01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_COMMIT_SIGNATURE_ENTID']
