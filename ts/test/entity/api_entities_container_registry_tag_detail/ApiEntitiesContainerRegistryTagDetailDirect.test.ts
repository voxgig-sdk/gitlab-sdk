...
describe('ApiEntitiesContainerRegistryTagDetailDirect', async () => {
  ...
  test('direct-load-api_entities_container_registry_tag_detail', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_container_registry_tag_detail', setup.live)) return
    if (skipIfMissingIds(t, setup, ["project_id01","repository_id01","tag_name01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_CONTAINER_REGISTRY_TAG_DETAIL_ENTID']
