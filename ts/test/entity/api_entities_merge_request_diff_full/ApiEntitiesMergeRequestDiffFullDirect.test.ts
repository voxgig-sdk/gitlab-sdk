...
describe('ApiEntitiesMergeRequestDiffFullDirect', async () => {
  ...
  test('direct-load-api_entities_merge_request_diff_full', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_merge_request_diff_full', setup.live)) return
    if (skipIfMissingIds(t, setup, ["merge_request_id01","project_id01","version_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_MERGE_REQUEST_DIFF_FULL_ENTID']
