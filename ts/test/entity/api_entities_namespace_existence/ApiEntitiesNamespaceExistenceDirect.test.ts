...
describe('ApiEntitiesNamespaceExistenceDirect', async () => {
  ...
  test('direct-list-api_entities_namespace_existence', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_namespace_existence', setup.live)) return
    if (skipIfMissingIds(t, setup, ["namespace01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_NAMESPACE_EXISTENCE_ENTID']
