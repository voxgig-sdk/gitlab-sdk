...
describe('ApiEntitiesAccessRequesterEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_access_requester.' + op, live)) return
    }
    ...
    api_entities_access_requester_ref01_data = await api_entities_access_requester_ref01_ent.create(api_entities_access_requester_ref01_data)
    ...
    const api_entities_access_requester_ref01_list = await api_entities_access_requester_ref01_ent.list(api_entities_access_requester_ref01_match)
    ...
    const api_entities_access_requester_ref01_resdata_up0 = await api_entities_access_requester_ref01_ent.update(api_entities_access_requester_ref01_data_up0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_ACCESS_REQUESTER_ENTID']
