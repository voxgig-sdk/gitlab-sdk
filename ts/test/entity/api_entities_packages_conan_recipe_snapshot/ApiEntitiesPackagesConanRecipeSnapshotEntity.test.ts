...
describe('ApiEntitiesPackagesConanRecipeSnapshotEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'api_entities_packages_conan_recipe_snapshot.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['api_entities_packages_conan_recipe_snapshot01','api_entities_packages_conan_recipe_snapshot02','api_entities_packages_conan_recipe_snapshot03','conan01','conan02','conan03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_RECIPE_SNAPSHOT_ENTID']
