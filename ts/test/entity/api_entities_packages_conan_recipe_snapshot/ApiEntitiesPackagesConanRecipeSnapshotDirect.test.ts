...
describe('ApiEntitiesPackagesConanRecipeSnapshotDirect', async () => {
  ...
  test('direct-load-api_entities_packages_conan_recipe_snapshot', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_packages_conan_recipe_snapshot', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","package_channel01","package_name01","package_username01","package_version01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_PACKAGES_CONAN_RECIPE_SNAPSHOT_ENTID']
