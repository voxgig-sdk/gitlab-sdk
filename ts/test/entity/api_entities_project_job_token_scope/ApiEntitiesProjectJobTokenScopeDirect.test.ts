...
describe('ApiEntitiesProjectJobTokenScopeDirect', async () => {
  ...
  test('direct-load-api_entities_project_job_token_scope', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_project_job_token_scope', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_PROJECT_JOB_TOKEN_SCOPE_ENTID']
