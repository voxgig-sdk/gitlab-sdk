...
describe('ApiEntitiesContainerRegistryTagDirect', async () => {
  ...
  test('direct-list-api_entities_container_registry_tag', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_container_registry_tag', setup.live)) return
    if (skipIfMissingIds(t, setup, ["project01","repository01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_CONTAINER_REGISTRY_TAG_ENTID']
