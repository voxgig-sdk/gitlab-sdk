...
describe('EeApiEntitiesSshCertificateDirect', async () => {
  ...
  test('direct-list-ee_api_entities_ssh_certificate', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ee_api_entities_ssh_certificate', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_EE_API_ENTITIES_SSH_CERTIFICATE_ENTID']
