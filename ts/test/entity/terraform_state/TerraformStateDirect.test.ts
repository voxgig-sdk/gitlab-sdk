...
describe('TerraformStateDirect', async () => {
  ...
  test('direct-load-terraform_state', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-terraform_state', setup.live)) return
    if (skipIfMissingIds(t, setup, ["project_id01","serial01","state_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_TERRAFORM_STATE_ENTID']
