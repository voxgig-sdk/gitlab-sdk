...
describe('ApiEntitiesNpmPackageTagDirect', async () => {
  ...
  test('direct-load-api_entities_npm_package_tag', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_npm_package_tag', setup.live)) return
    if (skipIfMissingIds(t, setup, ["group_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_NPM_PACKAGE_TAG_ENTID']
