...
describe('ApiEntitiesFeatureFlagUserListDirect', async () => {
  ...
  test('direct-load-api_entities_feature_flag_user_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_entities_feature_flag_user_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["iid01","project01"])) return
  ...
  test('direct-list-api_entities_feature_flag_user_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-api_entities_feature_flag_user_list', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GITLAB_TEST_API_ENTITIES_FEATURE_FLAG_USER_LIST_ENTID']
