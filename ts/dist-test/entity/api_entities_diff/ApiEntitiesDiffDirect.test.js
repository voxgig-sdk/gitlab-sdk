...
(0, node_test_1.describe)('ApiEntitiesDiffDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-api_entities_diff', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-api_entities_diff', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["merge_request01", "project01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.merge_request_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.merge_request_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-api_entities_diff', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-api_entities_diff', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["project01", "sha01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITLAB_TEST_API_ENTITIES_DIFF_ENTID'];
...
//# sourceMappingURL=ApiEntitiesDiffDirect.test.js.map