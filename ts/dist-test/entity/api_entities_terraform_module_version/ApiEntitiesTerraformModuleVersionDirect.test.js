...
(0, node_test_1.describe)('ApiEntitiesTerraformModuleVersionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-api_entities_terraform_module_version', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-api_entities_terraform_module_version', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["module_name01", "module_system01", "v101"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.module_name ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.module_name = candidateId;
    ...
    (0, node_test_1.test)('direct-list-api_entities_terraform_module_version', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-api_entities_terraform_module_version', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["module_name01", "module_system01", "v101"]))
            return;
        const { client, calls } = setup;
...
function liveScenariosActive() { return false && process.env.GITLAB_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.GitlabSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.GITLAB_APIKEY,
        }));
        let idmap = env['GITLAB_TEST_API_ENTITIES_TERRAFORM_MODULE_VERSION_ENTID'];
...
//# sourceMappingURL=ApiEntitiesTerraformModuleVersionDirect.test.js.map