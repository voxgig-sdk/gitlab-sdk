...
(0, node_test_1.describe)('ApiEntitiesResourceAccessTokenWithTokenEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when GITLAB_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('GITLAB_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.GitlabSDK.test();
        const ent = testsdk.ApiEntitiesResourceAccessTokenWithToken();
        (0, node_assert_1.default)(null != ent);
    });
    (0, node_test_1.test)('basic', async (t) => {
        const live = 'TRUE' === process.env.GITLAB_TEST_LIVE;
        for (const op of ['create']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'api_entities_resource_access_token_with_token.' + op, live))
                return;
        }
        ...
        // CREATE
        const api_entities_resource_access_token_with_token_ref01_ent = client.ApiEntitiesResourceAccessTokenWithToken();
        let api_entities_resource_access_token_with_token_ref01_data = setup.data.new.api_entities_resource_access_token_with_token['api_entities_resource_access_token_with_token_ref01'];
        api_entities_resource_access_token_with_token_ref01_data['project_id'] = setup.idmap['project01'];
        api_entities_resource_access_token_with_token_ref01_data = (await api_entities_resource_access_token_with_token_ref01_ent.create(api_entities_resource_access_token_with_token_ref01_data)).data();
        (0, node_assert_1.default)(null != api_entities_resource_access_token_with_token_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/api_entities_resource_access_token_with_token/ApiEntitiesResourceAccessTokenWithTokenTestData.json');
    ...
    let idmap = transform(['api_entities_resource_access_token_with_token01', 'api_entities_resource_access_token_with_token02', 'api_entities_resource_access_token_with_token03', 'group01', 'group02', 'group03', 'project01', 'project02', 'project03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    idmap = env['GITLAB_TEST_API_ENTITIES_RESOURCE_ACCESS_TOKEN_WITH_TOKEN_ENTID'];
...
//# sourceMappingURL=ApiEntitiesResourceAccessTokenWithTokenEntity.test.js.map