...
(0, node_test_1.describe)('ApiEntitiesBulkImportDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-api_entities_bulk_import', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-api_entities_bulk_import', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["bulk_import01", "entity01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                ...
                method: 'GET',
                params: {
                    bulk_import_id: setup.idmap['bulk_import01'],
                },
            });
            ...
            const candidateId = listArr[0]?.bulk_import_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.bulk_import_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-api_entities_bulk_import', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-api_entities_bulk_import', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["bulk_import01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITLAB_TEST_API_ENTITIES_BULK_IMPORT_ENTID'];
...
//# sourceMappingURL=ApiEntitiesBulkImportDirect.test.js.map