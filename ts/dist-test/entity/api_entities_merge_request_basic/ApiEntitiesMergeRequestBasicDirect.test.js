...
(0, node_test_1.describe)('ApiEntitiesMergeRequestBasicDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-api_entities_merge_request_basic', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-api_entities_merge_request_basic', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["group01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.group_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.group_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-api_entities_merge_request_basic', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-api_entities_merge_request_basic', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["deployment01", "project01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITLAB_TEST_API_ENTITIES_MERGE_REQUEST_BASIC_ENTID'];
...
//# sourceMappingURL=ApiEntitiesMergeRequestBasicDirect.test.js.map