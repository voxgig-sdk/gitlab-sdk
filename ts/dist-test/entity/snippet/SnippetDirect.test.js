...
(0, node_test_1.describe)('SnippetDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-snippet', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-snippet', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["file_id01", "file_path01", "id01"]))
            return;
        ...
        const result = await client.direct({
            path: 'api/v4/snippets/{id}/files/{file_id}/{file_path}/raw',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['GITLAB_TEST_SNIPPET_ENTID'];
