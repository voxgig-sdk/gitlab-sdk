...
{ "entity": "api_entities_project_with_access", "query": { "id": 1 } }
```
...
4. **Restart Claude Code.** The `gitlab_list` and `gitlab_load` tools now appear
   in new sessions. Ask the agent to *"list project using gitlab"*
   and it calls `gitlab_list` with `{"entity":"project"}`.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gitlab_load","arguments":{"entity":"api_entities_project_with_access","query":{"id":1}}}}'
```
