...
class ApiEntitiesNpmPackageTagEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "api_entities_npm_package_tag"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    ApiEntitiesNpmPackageTagEntity.new(@_client, opts)
  end
  ...
  # @return [ApiEntitiesNpmPackageTag, Hash] the current ApiEntitiesNpmPackageTag data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of ApiEntitiesNpmPackageTag fields)
  def match_get
  ...
  #
  # @param reqmatch [ApiEntitiesNpmPackageTagLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ApiEntitiesNpmPackageTag.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ApiEntitiesNpmPackageTag, Hash] the loaded ApiEntitiesNpmPackageTag; raises GitlabError on failure
