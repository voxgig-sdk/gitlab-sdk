...
class ApiEntitiesTreeObjectEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "api_entities_tree_object"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    ApiEntitiesTreeObjectEntity.new(@_client, opts)
  end
  ...
  # @return [ApiEntitiesTreeObject, Hash] the current ApiEntitiesTreeObject data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of ApiEntitiesTreeObject fields)
  def match_get
  ...
  #
  # @param reqmatch [ApiEntitiesTreeObjectLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ApiEntitiesTreeObject.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ApiEntitiesTreeObject, Hash] the loaded ApiEntitiesTreeObject; raises GitlabError on failure
