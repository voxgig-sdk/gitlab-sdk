...
class GoProxyEntity
  ...
  def make
    opts = @_entopts.dup
    GoProxyEntity.new(@_client, opts)
  end
  ...
  # @return [GoProxy, Hash] the current GoProxy data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of GoProxy fields)
  def match_get
  ...
  #
  # @param reqmatch [GoProxyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GoProxy.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GoProxy, Hash] the loaded GoProxy; raises GitlabError on failure
