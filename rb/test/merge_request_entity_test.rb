...
class MergeRequestEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_MERGE_REQUEST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    merge_request_ref01_data = nil
    if merge_request_ref01_data_raw.length > 0
...
def merge_request_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "merge_request", "MergeRequestTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_MERGE_REQUEST_ENTID"]
