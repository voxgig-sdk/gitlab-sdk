...
class DependencyProxyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_DEPENDENCY_PROXY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    dependency_proxy_ref01_data = nil
...
def dependency_proxy_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "dependency_proxy", "DependencyProxyTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_DEPENDENCY_PROXY_ENTID"]
