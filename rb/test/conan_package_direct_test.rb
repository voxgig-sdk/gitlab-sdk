...
class ConanPackageDirectTest < Minitest::Test
  def test_direct_load_conan_package
    setup = conan_package_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-conan_package", setup[:live] ? "live" : "unit")
    if _should_skip
...
def conan_package_direct_setup(mockres)
