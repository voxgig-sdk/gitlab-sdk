...
class ApiEntitiesPackagesConanRevisionDirectTest < Minitest::Test
  def test_direct_load_api_entities_packages_conan_revision
    setup = api_entities_packages_conan_revision_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-api_entities_packages_conan_revision", setup[:live] ? "live" : "unit")
    if _should_skip
...
def api_entities_packages_conan_revision_direct_setup(mockres)
