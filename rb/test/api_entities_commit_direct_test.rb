...
class ApiEntitiesCommitDirectTest < Minitest::Test
  def test_direct_list_api_entities_commit
    setup = api_entities_commit_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-api_entities_commit", setup[:live] ? "live" : "unit")
    if _should_skip
...
def api_entities_commit_direct_setup(mockres)
