...
class ApiEntitiesClustersAgentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = api_entities_clusters_agent_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "api_entities_clusters_agent." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_CLUSTERS_AGENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    api_entities_clusters_agent_ref01_ent = client.ApiEntitiesClustersAgent(nil)
...
def api_entities_clusters_agent_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "api_entities_clusters_agent", "ApiEntitiesClustersAgentTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_API_ENTITIES_CLUSTERS_AGENT_ENTID"]
