...
class ApiEntitiesRelatedIssueDirectTest < Minitest::Test
  def test_direct_list_api_entities_related_issue
    setup = api_entities_related_issue_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-api_entities_related_issue", setup[:live] ? "live" : "unit")
    if _should_skip
...
def api_entities_related_issue_direct_setup(mockres)
