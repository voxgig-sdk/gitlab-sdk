...
class GroupExportEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = group_export_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "group_export." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_GROUP_EXPORT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    group_export_ref01_ent = client.GroupExport(nil)
...
def group_export_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "group_export", "GroupExportTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_GROUP_EXPORT_ENTID"]
