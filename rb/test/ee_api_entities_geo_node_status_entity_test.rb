...
class EeApiEntitiesGeoNodeStatusEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = ee_api_entities_geo_node_status_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "ee_api_entities_geo_node_status." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_EE_API_ENTITIES_GEO_NODE_STATUS_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    ee_api_entities_geo_node_status_ref01_ent = client.EeApiEntitiesGeoNodeStatus(nil)
...
def ee_api_entities_geo_node_status_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "ee_api_entities_geo_node_status", "EeApiEntitiesGeoNodeStatusTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_EE_API_ENTITIES_GEO_NODE_STATUS_ENTID"]
