...
class ApiEntitiesDeployKeyEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ApiEntitiesDeployKey(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ApiEntitiesDeployKey(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = api_entities_deploy_key_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "api_entities_deploy_key." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_DEPLOY_KEY_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    api_entities_deploy_key_ref01_ent = client.ApiEntitiesDeployKey(nil)
    ...
    api_entities_deploy_key_ref01_list_result = api_entities_deploy_key_ref01_ent.list(api_entities_deploy_key_ref01_match, nil)
    assert api_entities_deploy_key_ref01_list_result.is_a?(Array)
...
def api_entities_deploy_key_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "api_entities_deploy_key", "ApiEntitiesDeployKeyTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_API_ENTITIES_DEPLOY_KEY_ENTID"]
