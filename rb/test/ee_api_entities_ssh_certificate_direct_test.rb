...
class EeApiEntitiesSshCertificateDirectTest < Minitest::Test
  def test_direct_list_ee_api_entities_ssh_certificate
    setup = ee_api_entities_ssh_certificate_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-ee_api_entities_ssh_certificate", setup[:live] ? "live" : "unit")
    if _should_skip
...
def ee_api_entities_ssh_certificate_direct_setup(mockres)
