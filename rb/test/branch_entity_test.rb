...
class BranchEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_BRANCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    branch_ref01_data = nil
    if branch_ref01_data_raw.length > 0
...
def branch_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITLAB_TEST_BRANCH_ENTID"]
