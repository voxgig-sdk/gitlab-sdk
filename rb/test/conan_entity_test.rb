...
class ConanEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_CONAN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    conan_ref01_data = nil
    if conan_ref01_data_raw.length > 0
...
def conan_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "conan", "ConanTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_CONAN_ENTID"]
