...
class ApiEntitiesLicenseDirectTest < Minitest::Test
  def test_direct_list_api_entities_license
    setup = api_entities_license_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-api_entities_license", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["api_entities_license01", "name01", "type01"].each do |_live_key|
...
def api_entities_license_direct_setup(mockres)
