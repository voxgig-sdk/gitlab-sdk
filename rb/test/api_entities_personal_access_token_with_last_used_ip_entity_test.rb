...
class ApiEntitiesPersonalAccessTokenWithLastUsedIpEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ApiEntitiesPersonalAccessTokenWithLastUsedIp(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ApiEntitiesPersonalAccessTokenWithLastUsedIp(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = api_entities_personal_access_token_with_last_used_ip_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "api_entities_personal_access_token_with_last_used_ip." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_PERSONAL_ACCESS_TOKEN_WITH_LAST_USED_IP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    api_entities_personal_access_token_with_last_used_ip_ref01_data = nil
    if api_entities_personal_access_token_with_last_used_ip_ref01_data_raw.length > 0
    ...
    # LIST
    api_entities_personal_access_token_with_last_used_ip_ref01_ent = client.ApiEntitiesPersonalAccessTokenWithLastUsedIp(nil)
    api_entities_personal_access_token_with_last_used_ip_ref01_match = {}

    api_entities_personal_access_token_with_last_used_ip_ref01_list_result = api_entities_personal_access_token_with_last_used_ip_ref01_ent.list(api_entities_personal_access_token_with_last_used_ip_ref01_match, nil)
    assert api_entities_personal_access_token_with_last_used_ip_ref01_list_result.is_a?(Array)
...
def api_entities_personal_access_token_with_last_used_ip_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "api_entities_personal_access_token_with_last_used_ip", "ApiEntitiesPersonalAccessTokenWithLastUsedIpTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_API_ENTITIES_PERSONAL_ACCESS_TOKEN_WITH_LAST_USED_IP_ENTID"]
