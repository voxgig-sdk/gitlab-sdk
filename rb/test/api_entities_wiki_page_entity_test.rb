...
class ApiEntitiesWikiPageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = api_entities_wiki_page_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "api_entities_wiki_page." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_WIKI_PAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    api_entities_wiki_page_ref01_ent = client.ApiEntitiesWikiPage(nil)
    ...
    api_entities_wiki_page_ref01_data_dt0_loaded = api_entities_wiki_page_ref01_ent.load(api_entities_wiki_page_ref01_match_dt0, nil)
    assert !api_entities_wiki_page_ref01_data_dt0_loaded.nil?

  end
end

def api_entities_wiki_page_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "api_entities_wiki_page", "ApiEntitiesWikiPageTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_API_ENTITIES_WIKI_PAGE_ENTID"]
