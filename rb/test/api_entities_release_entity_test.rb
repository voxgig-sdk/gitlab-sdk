...
class ApiEntitiesReleaseEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ApiEntitiesRelease(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ApiEntitiesRelease(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = api_entities_release_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "api_entities_release." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_RELEASE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    api_entities_release_ref01_ent = client.ApiEntitiesRelease(nil)
    ...
    assert !api_entities_release_ref01_data["id"].nil?
    ...
    assert_equal api_entities_release_ref01_resdata_up0["id"], api_entities_release_ref01_data_up0_up["id"]
    ...
    api_entities_release_ref01_data_dt0_loaded = api_entities_release_ref01_ent.load(api_entities_release_ref01_match_dt0, nil)
    api_entities_release_ref01_data_dt0_load_result = Helpers.to_map(api_entities_release_ref01_data_dt0_loaded.respond_to?(:data_get) ? api_entities_release_ref01_data_dt0_loaded.data_get : api_entities_release_ref01_data_dt0_loaded)
    assert !api_entities_release_ref01_data_dt0_load_result.nil?
    assert_equal api_entities_release_ref01_data_dt0_load_result["id"], api_entities_release_ref01_data["id"]

  end
end

def api_entities_release_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "api_entities_release", "ApiEntitiesReleaseTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_API_ENTITIES_RELEASE_ENTID"]
