...
class NugetEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_NUGET_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    nuget_ref01_data = nil
    if nuget_ref01_data_raw.length > 0
...
def nuget_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITLAB_TEST_NUGET_ENTID"]
