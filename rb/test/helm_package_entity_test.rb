...
class HelmPackageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = helm_package_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "helm_package." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_HELM_PACKAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    helm_package_ref01_ent = client.HelmPackage(nil)
    ...
    helm_package_ref01_data_dt0_loaded = helm_package_ref01_ent.load(helm_package_ref01_match_dt0, nil)
    assert !helm_package_ref01_data_dt0_loaded.nil?

  end
end

def helm_package_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "helm_package", "HelmPackageTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_HELM_PACKAGE_ENTID"]
