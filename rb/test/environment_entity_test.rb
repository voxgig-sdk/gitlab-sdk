...
class EnvironmentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = environment_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "environment." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_ENVIRONMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    environment_ref01_ent = client.Environment(nil)
    ...
    assert !environment_ref01_data["id"].nil?
...
def environment_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "environment", "EnvironmentTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_ENVIRONMENT_ENTID"]
