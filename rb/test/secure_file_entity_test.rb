...
class SecureFileEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_SECURE_FILE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    secure_file_ref01_data = nil
    if secure_file_ref01_data_raw.length > 0
...
def secure_file_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "secure_file", "SecureFileTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_SECURE_FILE_ENTID"]
