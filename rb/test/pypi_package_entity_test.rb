...
class PypiPackageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = pypi_package_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "pypi_package." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_PYPI_PACKAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    pypi_package_ref01_ent = client.PypiPackage(nil)
    ...
    pypi_package_ref01_data_dt0_loaded = pypi_package_ref01_ent.load(pypi_package_ref01_match_dt0, nil)
    assert !pypi_package_ref01_data_dt0_loaded.nil?

  end
end

def pypi_package_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "pypi_package", "PypiPackageTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_PYPI_PACKAGE_ENTID"]
