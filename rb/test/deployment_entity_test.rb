...
class DeploymentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_DEPLOYMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # Bootstrap entity data from existing test data.
    deployment_ref01_data_raw = Vs.items(Helpers.to_map(
      Vs.getpath(setup[:data], "existing.deployment")))
    deployment_ref01_data = nil
    if deployment_ref01_data_raw.length > 0
      deployment_ref01_data = Helpers.to_map(deployment_ref01_data_raw[0][1])
    end
    ...
    deployment_ref01_ent.remove(deployment_ref01_match_rm0, nil)
...
def deployment_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITLAB_TEST_DEPLOYMENT_ENTID"]
