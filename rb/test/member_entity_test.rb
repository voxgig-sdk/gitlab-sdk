...
class MemberEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_MEMBER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    member_ref01_ent.remove(member_ref01_match_rm0, nil)
...
def member_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITLAB_TEST_MEMBER_ENTID"]
