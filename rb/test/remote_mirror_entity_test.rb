...
class RemoteMirrorEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_REMOTE_MIRROR_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def remote_mirror_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "remote_mirror", "RemoteMirrorTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_REMOTE_MIRROR_ENTID"]
