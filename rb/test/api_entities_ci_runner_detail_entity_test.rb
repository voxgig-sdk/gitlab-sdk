...
class ApiEntitiesCiRunnerDetailEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = api_entities_ci_runner_detail_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "api_entities_ci_runner_detail." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_CI_RUNNER_DETAIL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    api_entities_ci_runner_detail_ref01_data = nil
    if api_entities_ci_runner_detail_ref01_data_raw.length > 0
      api_entities_ci_runner_detail_ref01_data = Helpers.to_map(api_entities_ci_runner_detail_ref01_data_raw[0][1])
    end

    # UPDATE
    api_entities_ci_runner_detail_ref01_ent = client.ApiEntitiesCiRunnerDetail(nil)
...
def api_entities_ci_runner_detail_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "api_entities_ci_runner_detail", "ApiEntitiesCiRunnerDetailTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_API_ENTITIES_CI_RUNNER_DETAIL_ENTID"]
