...
class FeatureFlagEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = feature_flag_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "feature_flag." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_FEATURE_FLAG_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    feature_flag_ref01_ent = client.FeatureFlag(nil)
...
def feature_flag_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "feature_flag", "FeatureFlagTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_FEATURE_FLAG_ENTID"]
