...
class ApiEntitiesFeatureEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ApiEntitiesFeature(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ApiEntitiesFeature(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = api_entities_feature_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "api_entities_feature." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_FEATURE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    api_entities_feature_ref01_ent = client.ApiEntitiesFeature(nil)
...
def api_entities_feature_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "api_entities_feature", "ApiEntitiesFeatureTestData.json")
  ...
  entid_env_raw = ENV["GITLAB_TEST_API_ENTITIES_FEATURE_ENTID"]
