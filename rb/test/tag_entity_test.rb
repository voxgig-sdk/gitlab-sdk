...
class TagEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GITLAB_TEST_TAG_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    tag_ref01_data = nil
    if tag_ref01_data_raw.length > 0
...
def tag_basic_setup(extra)
  ...
  entid_env_raw = ENV["GITLAB_TEST_TAG_ENTID"]
