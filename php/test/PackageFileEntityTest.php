...
class PackageFileEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GitlabSDK::test(null, null);
        $ent = $testsdk->PackageFile(null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_PACKAGE_FILE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $package_file_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.package_file")));
        $package_file_ref01_data = null;
        if (count($package_file_ref01_data_raw) > 0) {
            $package_file_ref01_data = Helpers::to_map($package_file_ref01_data_raw[0][1]);
        }
...
function package_file_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/package_file/PackageFileTestData.json';
    ...
    $entid_env_raw = getenv("GITLAB_TEST_PACKAGE_FILE_ENTID");
