...
class ContainerRegistryEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GitlabSDK::test(null, null);
        $ent = $testsdk->ContainerRegistry(null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_CONTAINER_REGISTRY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $container_registry_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.container_registry")));
        $container_registry_ref01_data = null;
...
function container_registry_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/container_registry/ContainerRegistryTestData.json';
    ...
    $entid_env_raw = getenv("GITLAB_TEST_CONTAINER_REGISTRY_ENTID");
