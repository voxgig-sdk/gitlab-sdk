...
class GenericPackageEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GitlabSDK::test(null, null);
        $ent = $testsdk->GenericPackage(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = generic_package_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "generic_package." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_GENERIC_PACKAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $generic_package_ref01_data = null;
        if (count($generic_package_ref01_data_raw) > 0) {
            $generic_package_ref01_data = Helpers::to_map($generic_package_ref01_data_raw[0][1]);
        }

        // UPDATE
        $generic_package_ref01_ent = $client->GenericPackage(null);
        ...
        $generic_package_ref01_data_dt0_loaded = $generic_package_ref01_ent->load($generic_package_ref01_match_dt0, null);
        $this->assertNotNull($generic_package_ref01_data_dt0_loaded);
...
function generic_package_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/generic_package/GenericPackageTestData.json';
    ...
    $entid_env_raw = getenv("GITLAB_TEST_GENERIC_PACKAGE_ENTID");
