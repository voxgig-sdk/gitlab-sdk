...
// ApiEntitiesLicense direct test
...
class ApiEntitiesLicenseDirectTest extends TestCase
{
    public function test_direct_list_api_entities_license(): void
    {
        $setup = api_entities_license_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-api_entities_license", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["api_entities_license01", "name01", "type01"] as $_liveKey) {
...
function api_entities_license_direct_setup($mockres)
{
