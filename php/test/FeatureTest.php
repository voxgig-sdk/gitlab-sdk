...
class FtCtrl extends GitlabControl
{
...
class FtClient
{
    ...
    private GitlabUtility $utility;

    public function __construct(GitlabUtility $utility, array $options)
    {
    ...
    public function get_utility(): GitlabUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GitlabContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GitlabContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GitlabConfig::make_feature($name);
    ...
    private static function buildUrl(GitlabSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GitlabError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GitlabResponse($fetched);
        }
    ...
    private function fail(GitlabContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GitlabContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
