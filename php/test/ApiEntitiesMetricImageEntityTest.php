...
class ApiEntitiesMetricImageEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GitlabSDK::test(null, null);
        $ent = $testsdk->ApiEntitiesMetricImage(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->ApiEntitiesMetricImage(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->ApiEntitiesMetricImage(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = api_entities_metric_image_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "api_entities_metric_image." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_METRIC_IMAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $api_entities_metric_image_ref01_ent = $client->ApiEntitiesMetricImage(null);
        ...
        $api_entities_metric_image_ref01_list_result = $api_entities_metric_image_ref01_ent->list($api_entities_metric_image_ref01_match, null);
        $this->assertIsArray($api_entities_metric_image_ref01_list_result);
...
function api_entities_metric_image_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/api_entities_metric_image/ApiEntitiesMetricImageTestData.json';
    ...
    $entid_env_raw = getenv("GITLAB_TEST_API_ENTITIES_METRIC_IMAGE_ENTID");
    ...
    if (!isset($idmap_resolved["alert_management_alert_id"])) {
