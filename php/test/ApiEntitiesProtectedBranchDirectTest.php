...
class ApiEntitiesProtectedBranchDirectTest extends TestCase
{
    public function test_direct_list_api_entities_protected_branch(): void
    {
        $setup = api_entities_protected_branch_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-api_entities_protected_branch", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_api_entities_protected_branch(): void
    {
        $setup = api_entities_protected_branch_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-api_entities_protected_branch", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
