...
class ApplicationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_APPLICATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $application_ref01_data = null;
        if (count($application_ref01_data_raw) > 0) {
        ...
        $application_ref01_ent->remove($application_ref01_match_rm0, null);
...
function application_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GITLAB_TEST_APPLICATION_ENTID");
