...
class MemberEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_MEMBER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function member_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["member01", "member02", "member03", "group01", "group02", "group03", "project01", "project02", "project03"] as $k) {
    ...
    $entid_env_raw = getenv("GITLAB_TEST_MEMBER_ENTID");
