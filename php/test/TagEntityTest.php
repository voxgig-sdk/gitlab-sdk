...
class TagEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_TAG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function tag_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["tag01", "tag02", "tag03", "project01", "project02", "project03"] as $k) {
    ...
    $entid_env_raw = getenv("GITLAB_TEST_TAG_ENTID");
