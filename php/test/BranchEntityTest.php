...
class BranchEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_BRANCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $branch_ref01_data = null;
        if (count($branch_ref01_data_raw) > 0) {
...
function branch_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GITLAB_TEST_BRANCH_ENTID");
