...
class DebianEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GitlabSDK::test(null, null);
        $ent = $testsdk->Debian(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = debian_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "debian." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_DEBIAN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $debian_ref01_data = null;
        if (count($debian_ref01_data_raw) > 0) {
            $debian_ref01_data = Helpers::to_map($debian_ref01_data_raw[0][1]);
        }

        // UPDATE
        $debian_ref01_ent = $client->Debian(null);
        ...
        $this->assertEquals($debian_ref01_resdata_up0["id"], $debian_ref01_data_up0_up["id"]);
...
function debian_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/debian/DebianTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["debian01", "debian02", "debian03", "project01", "project02", "project03"] as $k) {
    ...
    $entid_env_raw = getenv("GITLAB_TEST_DEBIAN_ENTID");
