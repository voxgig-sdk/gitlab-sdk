...
class TerraformStateEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GitlabSDK::test(null, null);
        $ent = $testsdk->TerraformState(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = terraform_state_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "terraform_state." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_TERRAFORM_STATE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $terraform_state_ref01_ent = $client->TerraformState(null);
...
function terraform_state_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/terraform_state/TerraformStateTestData.json';
    ...
    $entid_env_raw = getenv("GITLAB_TEST_TERRAFORM_STATE_ENTID");
