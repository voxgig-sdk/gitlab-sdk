...
class EeApiEntitiesBillableMembershipEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GitlabSDK::test(null, null);
        $ent = $testsdk->EeApiEntitiesBillableMembership(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = ee_api_entities_billable_membership_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "ee_api_entities_billable_membership." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_EE_API_ENTITIES_BILLABLE_MEMBERSHIP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ee_api_entities_billable_membership_ref01_data = null;
        if (count($ee_api_entities_billable_membership_ref01_data_raw) > 0) {
...
function ee_api_entities_billable_membership_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/ee_api_entities_billable_membership/EeApiEntitiesBillableMembershipTestData.json';
    ...
    $entid_env_raw = getenv("GITLAB_TEST_EE_API_ENTITIES_BILLABLE_MEMBERSHIP_ENTID");
