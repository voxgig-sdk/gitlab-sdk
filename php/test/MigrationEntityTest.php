...
class MigrationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Migration(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = migration_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "migration." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_MIGRATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $migration_ref01_data = Helpers::to_map(Vs::getprop(
...
function migration_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/migration/MigrationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["migration01", "migration02", "migration03", "timestamp01"] as $k) {
    ...
    $entid_env_raw = getenv("GITLAB_TEST_MIGRATION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GITLAB_TEST_MIGRATION_ENTID"]);
