...
describe("MergeRequestEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:MergeRequest(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_MERGE_REQUEST_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local merge_request_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.merge_request")))
    local merge_request_ref01_data = nil
    if #merge_request_ref01_data_raw > 0 then
      merge_request_ref01_data = helpers.to_map(merge_request_ref01_data_raw[1][2])
    end
...
function merge_request_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/merge_request/MergeRequestTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read merge_request test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_MERGE_REQUEST_ENTID")
