-- EeApiEntitiesAuditEvent direct test
...
describe("EeApiEntitiesAuditEventDirect", function()
  it("should direct-list-ee_api_entities_audit_event", function()
    local setup = ee_api_entities_audit_event_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-ee_api_entities_audit_event", setup.live and "live" or "unit")
  ...
  it("should direct-load-ee_api_entities_audit_event", function()
    local setup = ee_api_entities_audit_event_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-ee_api_entities_audit_event", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function ee_api_entities_audit_event_direct_setup(mockres)
