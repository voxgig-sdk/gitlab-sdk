...
describe("NpmPackageEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:NpmPackage(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = npm_package_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "npm_package." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_NPM_PACKAGE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local npm_package_ref01_ent = client:NpmPackage(nil)
    local npm_package_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- UPDATE
    local npm_package_ref01_data_up0_up = {
    }
    ...
    -- LOAD
    local npm_package_ref01_match_dt0 = {}
    local npm_package_ref01_data_dt0_loaded, err = npm_package_ref01_ent:load(npm_package_ref01_match_dt0, nil)
    assert.is_nil(err)
...
function npm_package_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/npm_package/NpmPackageTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read npm_package test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_NPM_PACKAGE_ENTID")
