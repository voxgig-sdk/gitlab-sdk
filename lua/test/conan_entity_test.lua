...
describe("ConanEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Conan(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_CONAN_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local conan_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.conan")))
    local conan_ref01_data = nil
...
function conan_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/conan/ConanTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read conan test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_CONAN_ENTID")
