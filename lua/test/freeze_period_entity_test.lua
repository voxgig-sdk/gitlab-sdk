...
describe("FreezePeriodEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:FreezePeriod(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_FREEZE_PERIOD_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local freeze_period_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.freeze_period")))
    local freeze_period_ref01_data = nil
...
function freeze_period_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/freeze_period/FreezePeriodTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read freeze_period test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_FREEZE_PERIOD_ENTID")
