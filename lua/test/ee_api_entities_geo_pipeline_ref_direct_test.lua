-- EeApiEntitiesGeoPipelineRef direct test
...
describe("EeApiEntitiesGeoPipelineRefDirect", function()
  it("should direct-list-ee_api_entities_geo_pipeline_ref", function()
    local setup = ee_api_entities_geo_pipeline_ref_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-ee_api_entities_geo_pipeline_ref", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"gl_repository01"}) do
...
function ee_api_entities_geo_pipeline_ref_direct_setup(mockres)
