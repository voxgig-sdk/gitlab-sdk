...
describe("EeApiEntitiesIssuableMetricImageEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EeApiEntitiesIssuableMetricImage(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = ee_api_entities_issuable_metric_image_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "update", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "ee_api_entities_issuable_metric_image." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_EE_API_ENTITIES_ISSUABLE_METRIC_IMAGE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local ee_api_entities_issuable_metric_image_ref01_ent = client:EeApiEntitiesIssuableMetricImage(nil)
    local ee_api_entities_issuable_metric_image_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- UPDATE
    local ee_api_entities_issuable_metric_image_ref01_data_up0_up = {
      id = ee_api_entities_issuable_metric_image_ref01_data["id"],
      ["issue_id"] = setup.idmap["issue_id"],
      ["project_id"] = setup.idmap["project_id"],
    }

    local ee_api_entities_issuable_metric_image_ref01_markdef_up0_name = "created_at"
    local ee_api_entities_issuable_metric_image_ref01_markdef_up0_value = "Mark01-ee_api_entities_issuable_metric_image_ref01_" .. tostring(setup.now)
    ...
    -- REMOVE
    local ee_api_entities_issuable_metric_image_ref01_match_rm0 = {
      id = ee_api_entities_issuable_metric_image_ref01_data["id"],
    }
    local _, err = ee_api_entities_issuable_metric_image_ref01_ent:remove(ee_api_entities_issuable_metric_image_ref01_match_rm0, nil)
    assert.is_nil(err)
...
function ee_api_entities_issuable_metric_image_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/ee_api_entities_issuable_metric_image/EeApiEntitiesIssuableMetricImageTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read ee_api_entities_issuable_metric_image test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_EE_API_ENTITIES_ISSUABLE_METRIC_IMAGE_ENTID")
  ...
  if idmap_resolved["issue_id"] == nil then
