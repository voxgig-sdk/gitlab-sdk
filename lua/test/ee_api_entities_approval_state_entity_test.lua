...
describe("EeApiEntitiesApprovalStateEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EeApiEntitiesApprovalState(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = ee_api_entities_approval_state_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "ee_api_entities_approval_state." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_EE_API_ENTITIES_APPROVAL_STATE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local ee_api_entities_approval_state_ref01_ent = client:EeApiEntitiesApprovalState(nil)
    local ee_api_entities_approval_state_ref01_data = helpers.to_map(vs.getprop(
...
function ee_api_entities_approval_state_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/ee_api_entities_approval_state/EeApiEntitiesApprovalStateTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read ee_api_entities_approval_state test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_EE_API_ENTITIES_APPROVAL_STATE_ENTID")
