...
describe("DebianPackageEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:DebianPackage(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = debian_package_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "debian_package." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_DEBIAN_PACKAGE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local debian_package_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.debian_package")))
    local debian_package_ref01_data = nil
    if #debian_package_ref01_data_raw > 0 then
      debian_package_ref01_data = helpers.to_map(debian_package_ref01_data_raw[1][2])
    end
...
function debian_package_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/debian_package/DebianPackageTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read debian_package test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_DEBIAN_PACKAGE_ENTID")
