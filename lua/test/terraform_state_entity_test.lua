...
describe("TerraformStateEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:TerraformState(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = terraform_state_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "terraform_state." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_TERRAFORM_STATE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local terraform_state_ref01_ent = client:TerraformState(nil)
    local terraform_state_ref01_data = helpers.to_map(vs.getprop(
...
function terraform_state_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/terraform_state/TerraformStateTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read terraform_state test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_TERRAFORM_STATE_ENTID")
