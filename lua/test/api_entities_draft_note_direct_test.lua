-- ApiEntitiesDraftNote direct test
...
describe("ApiEntitiesDraftNoteDirect", function()
  it("should direct-list-api_entities_draft_note", function()
    local setup = api_entities_draft_note_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_draft_note", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"merge_request01", "project01"}) do
  ...
  it("should direct-load-api_entities_draft_note", function()
    local setup = api_entities_draft_note_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-api_entities_draft_note", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function api_entities_draft_note_direct_setup(mockres)
