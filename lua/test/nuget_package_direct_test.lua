...
describe("NugetPackageDirect", function()
  it("should direct-list-nuget_package", function()
    local setup = nuget_package_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-nuget_package", setup.live and "live" or "unit")
  ...
  it("should direct-load-nuget_package", function()
    local setup = nuget_package_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-nuget_package", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
