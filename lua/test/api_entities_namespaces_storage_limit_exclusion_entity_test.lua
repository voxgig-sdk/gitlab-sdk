...
describe("ApiEntitiesNamespacesStorageLimitExclusionEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:ApiEntitiesNamespacesStorageLimitExclusion(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = api_entities_namespaces_storage_limit_exclusion_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "api_entities_namespaces_storage_limit_exclusion." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_NAMESPACES_STORAGE_LIMIT_EXCLUSION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local api_entities_namespaces_storage_limit_exclusion_ref01_ent = client:ApiEntitiesNamespacesStorageLimitExclusion(nil)
    local api_entities_namespaces_storage_limit_exclusion_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LOAD
    local api_entities_namespaces_storage_limit_exclusion_ref01_match_dt0 = {
...
function api_entities_namespaces_storage_limit_exclusion_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/api_entities_namespaces_storage_limit_exclusion/ApiEntitiesNamespacesStorageLimitExclusionTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read api_entities_namespaces_storage_limit_exclusion test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_API_ENTITIES_NAMESPACES_STORAGE_LIMIT_EXCLUSION_ENTID")
