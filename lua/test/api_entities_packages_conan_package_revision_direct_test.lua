...
describe("ApiEntitiesPackagesConanPackageRevisionDirect", function()
  it("should direct-list-api_entities_packages_conan_package_revision", function()
    local setup = api_entities_packages_conan_package_revision_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_packages_conan_package_revision", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"conan01", "conan_package_reference01", "package_channel01", "package_username01", "package_version01", "project01", "revision01"}) do
...
function api_entities_packages_conan_package_revision_direct_setup(mockres)
