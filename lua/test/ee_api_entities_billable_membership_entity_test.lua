...
describe("EeApiEntitiesBillableMembershipEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EeApiEntitiesBillableMembership(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = ee_api_entities_billable_membership_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "ee_api_entities_billable_membership." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_EE_API_ENTITIES_BILLABLE_MEMBERSHIP_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local ee_api_entities_billable_membership_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.ee_api_entities_billable_membership")))
    local ee_api_entities_billable_membership_ref01_data = nil
    if #ee_api_entities_billable_membership_ref01_data_raw > 0 then
      ee_api_entities_billable_membership_ref01_data = helpers.to_map(ee_api_entities_billable_membership_ref01_data_raw[1][2])
    end
...
function ee_api_entities_billable_membership_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/ee_api_entities_billable_membership/EeApiEntitiesBillableMembershipTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read ee_api_entities_billable_membership test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_EE_API_ENTITIES_BILLABLE_MEMBERSHIP_ENTID")
