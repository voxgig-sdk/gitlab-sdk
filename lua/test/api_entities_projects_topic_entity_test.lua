...
describe("ApiEntitiesProjectsTopicEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:ApiEntitiesProjectsTopic(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = api_entities_projects_topic_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "api_entities_projects_topic." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_API_ENTITIES_PROJECTS_TOPIC_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local api_entities_projects_topic_ref01_ent = client:ApiEntitiesProjectsTopic(nil)
    local api_entities_projects_topic_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- UPDATE
    local api_entities_projects_topic_ref01_data_up0_up = {
      id = api_entities_projects_topic_ref01_data["id"],
    }

    local api_entities_projects_topic_ref01_markdef_up0_name = "avatar_url"
    local api_entities_projects_topic_ref01_markdef_up0_value = "Mark01-api_entities_projects_topic_ref01_" .. tostring(setup.now)
    ...
    -- LOAD
    local api_entities_projects_topic_ref01_match_dt0 = {
      id = api_entities_projects_topic_ref01_data["id"],
    }
...
function api_entities_projects_topic_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/api_entities_projects_topic/ApiEntitiesProjectsTopicTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read api_entities_projects_topic test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_API_ENTITIES_PROJECTS_TOPIC_ENTID")
