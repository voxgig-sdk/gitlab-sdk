...
describe("DeploymentEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_DEPLOYMENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local deployment_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.deployment")))
    local deployment_ref01_data = nil
    if #deployment_ref01_data_raw > 0 then
      deployment_ref01_data = helpers.to_map(deployment_ref01_data_raw[1][2])
    end

    -- REMOVE
    local deployment_ref01_ent = client:Deployment(nil)
    local deployment_ref01_match_rm0 = {
      id = deployment_ref01_data["id"],
    }
    local _, err = deployment_ref01_ent:remove(deployment_ref01_match_rm0, nil)
    assert.is_nil(err)
...
function deployment_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_DEPLOYMENT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["GITLAB_TEST_DEPLOYMENT_ENTID"])
