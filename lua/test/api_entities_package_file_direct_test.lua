-- ApiEntitiesPackageFile direct test
...
describe("ApiEntitiesPackageFileDirect", function()
  it("should direct-list-api_entities_package_file", function()
    local setup = api_entities_package_file_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_package_file", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"package01", "project01"}) do
...
function api_entities_package_file_direct_setup(mockres)
