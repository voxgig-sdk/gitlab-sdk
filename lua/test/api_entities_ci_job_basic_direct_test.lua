-- ApiEntitiesCiJobBasic direct test
...
describe("ApiEntitiesCiJobBasicDirect", function()
  it("should direct-list-api_entities_ci_job_basic", function()
    local setup = api_entities_ci_job_basic_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_ci_job_basic", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"key01", "project01"}) do
...
function api_entities_ci_job_basic_direct_setup(mockres)
