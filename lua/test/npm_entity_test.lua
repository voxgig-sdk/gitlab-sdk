...
describe("NpmEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GITLAB_TEST_NPM_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local npm_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.npm")))
    local npm_ref01_data = nil
    if #npm_ref01_data_raw > 0 then
      npm_ref01_data = helpers.to_map(npm_ref01_data_raw[1][2])
    end
    ...
    assert.are.equal(npm_ref01_resdata_up0["id"], npm_ref01_data_up0_up["id"])
...
function npm_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GITLAB_TEST_NPM_ENTID")
