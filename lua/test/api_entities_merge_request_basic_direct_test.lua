-- ApiEntitiesMergeRequestBasic direct test
...
describe("ApiEntitiesMergeRequestBasicDirect", function()
  it("should direct-list-api_entities_merge_request_basic", function()
    local setup = api_entities_merge_request_basic_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-api_entities_merge_request_basic", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"deployment01", "project01"}) do
  ...
  it("should direct-load-api_entities_merge_request_basic", function()
    local setup = api_entities_merge_request_basic_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-api_entities_merge_request_basic", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function api_entities_merge_request_basic_direct_setup(mockres)
