-- Gitlab SDK ProtectedTag entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProtectedTagEntity = {}
ProtectedTagEntity.__index = ProtectedTagEntity


function ProtectedTagEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProtectedTagEntity)
  self._name = "protected_tag"
  self._client = client
...
function ProtectedTagEntity:get_name()
  return self._name
end


function ProtectedTagEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProtectedTagEntity.new(self._client, opts)
end
...
function ProtectedTagEntity:mark_deleted()
  self._deleted = true
end


function ProtectedTagEntity:deleted()
  return true == self._deleted
end


function ProtectedTagEntity:data_set(args)
  if args ~= nil then
...
function ProtectedTagEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProtectedTagEntity:match_set(args)
  if args ~= nil then
...
function ProtectedTagEntity:match_get()
...
function ProtectedTagEntity:stream(action, args, callopts)
...
---@param reqmatch ProtectedTagRemoveMatch
---@param ctrl? table
---@return ProtectedTag
---@return string? err
function ProtectedTagEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ProtectedTagEntity:_run_op(ctx, post_done)
  local utility = self._utility
