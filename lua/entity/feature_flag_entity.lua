...
local FeatureFlagEntity = {}
FeatureFlagEntity.__index = FeatureFlagEntity


function FeatureFlagEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FeatureFlagEntity)
  self._name = "feature_flag"
  self._client = client
...
function FeatureFlagEntity:get_name()
  return self._name
end


function FeatureFlagEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FeatureFlagEntity.new(self._client, opts)
end
...
function FeatureFlagEntity:mark_deleted()
  self._deleted = true
end


function FeatureFlagEntity:deleted()
  return true == self._deleted
end


function FeatureFlagEntity:data_set(args)
  if args ~= nil then
...
function FeatureFlagEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FeatureFlagEntity:match_set(args)
  if args ~= nil then
...
function FeatureFlagEntity:match_get()
...
function FeatureFlagEntity:stream(action, args, callopts)
...
---@param reqmatch FeatureFlagLoadMatch
---@param ctrl? table
---@return FeatureFlag
---@return string? err
function FeatureFlagEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata FeatureFlagCreateData
---@param ctrl? table
---@return FeatureFlag
---@return string? err
function FeatureFlagEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch FeatureFlagRemoveMatch
---@param ctrl? table
---@return FeatureFlag
---@return string? err
function FeatureFlagEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function FeatureFlagEntity:_run_op(ctx, post_done)
  local utility = self._utility
