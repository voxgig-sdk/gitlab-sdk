-- Gitlab SDK GroupImport entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GroupImportEntity = {}
GroupImportEntity.__index = GroupImportEntity


function GroupImportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GroupImportEntity)
  self._name = "group_import"
  self._client = client
...
function GroupImportEntity:get_name()
  return self._name
end


function GroupImportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GroupImportEntity.new(self._client, opts)
end
...
function GroupImportEntity:mark_deleted()
  self._deleted = true
end


function GroupImportEntity:deleted()
  return true == self._deleted
end


function GroupImportEntity:data_set(args)
  if args ~= nil then
...
function GroupImportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GroupImportEntity:match_set(args)
  if args ~= nil then
...
function GroupImportEntity:match_get()
...
function GroupImportEntity:stream(action, args, callopts)
...
---@param reqdata GroupImportCreateData
---@param ctrl? table
---@return GroupImport
---@return string? err
function GroupImportEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function GroupImportEntity:_run_op(ctx, post_done)
  local utility = self._utility
