...
local GroupExportEntity = {}
GroupExportEntity.__index = GroupExportEntity


function GroupExportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GroupExportEntity)
  self._name = "group_export"
  self._client = client
...
function GroupExportEntity:get_name()
  return self._name
end


function GroupExportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GroupExportEntity.new(self._client, opts)
end
...
function GroupExportEntity:mark_deleted()
  self._deleted = true
end


function GroupExportEntity:deleted()
  return true == self._deleted
end


function GroupExportEntity:data_set(args)
  if args ~= nil then
...
function GroupExportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GroupExportEntity:match_set(args)
  if args ~= nil then
...
function GroupExportEntity:match_get()
...
function GroupExportEntity:stream(action, args, callopts)
...
---@param reqmatch GroupExportLoadMatch
---@param ctrl? table
---@return GroupExport
---@return string? err
function GroupExportEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata GroupExportCreateData
---@param ctrl? table
---@return GroupExport
---@return string? err
function GroupExportEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function GroupExportEntity:_run_op(ctx, post_done)
  local utility = self._utility
