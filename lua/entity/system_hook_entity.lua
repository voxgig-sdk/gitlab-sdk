-- Gitlab SDK SystemHook entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SystemHookEntity = {}
SystemHookEntity.__index = SystemHookEntity


function SystemHookEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SystemHookEntity)
  self._name = "system_hook"
  self._client = client
...
function SystemHookEntity:get_name()
  return self._name
end


function SystemHookEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SystemHookEntity.new(self._client, opts)
end
...
function SystemHookEntity:mark_deleted()
  self._deleted = true
end


function SystemHookEntity:deleted()
  return true == self._deleted
end


function SystemHookEntity:data_set(args)
  if args ~= nil then
...
function SystemHookEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SystemHookEntity:match_set(args)
  if args ~= nil then
...
function SystemHookEntity:match_get()
...
function SystemHookEntity:stream(action, args, callopts)
...
---@param reqmatch SystemHookRemoveMatch
---@param ctrl? table
---@return SystemHook
---@return string? err
function SystemHookEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function SystemHookEntity:_run_op(ctx, post_done)
  local utility = self._utility
