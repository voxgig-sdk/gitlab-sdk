-- Gitlab SDK UnleashApi entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local UnleashApiEntity = {}
UnleashApiEntity.__index = UnleashApiEntity


function UnleashApiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UnleashApiEntity)
  self._name = "unleash_api"
  self._client = client
...
function UnleashApiEntity:get_name()
  return self._name
end


function UnleashApiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UnleashApiEntity.new(self._client, opts)
end
...
function UnleashApiEntity:mark_deleted()
  self._deleted = true
end


function UnleashApiEntity:deleted()
  return true == self._deleted
end


function UnleashApiEntity:data_set(args)
  if args ~= nil then
...
function UnleashApiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UnleashApiEntity:match_set(args)
  if args ~= nil then
...
function UnleashApiEntity:match_get()
...
function UnleashApiEntity:stream(action, args, callopts)
...
---@param reqmatch UnleashApiLoadMatch
---@param ctrl? table
---@return UnleashApi
---@return string? err
function UnleashApiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function UnleashApiEntity:_run_op(ctx, post_done)
  local utility = self._utility
