-- Gitlab SDK AwardEmoji entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AwardEmojiEntity = {}
AwardEmojiEntity.__index = AwardEmojiEntity


function AwardEmojiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AwardEmojiEntity)
  self._name = "award_emoji"
  self._client = client
...
function AwardEmojiEntity:get_name()
  return self._name
end


function AwardEmojiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AwardEmojiEntity.new(self._client, opts)
end
...
function AwardEmojiEntity:mark_deleted()
  self._deleted = true
end


function AwardEmojiEntity:deleted()
  return true == self._deleted
end


function AwardEmojiEntity:data_set(args)
  if args ~= nil then
...
function AwardEmojiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AwardEmojiEntity:match_set(args)
  if args ~= nil then
...
function AwardEmojiEntity:match_get()
...
function AwardEmojiEntity:stream(action, args, callopts)
...
---@param reqmatch AwardEmojiRemoveMatch
---@param ctrl? table
---@return AwardEmoji
---@return string? err
function AwardEmojiEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function AwardEmojiEntity:_run_op(ctx, post_done)
  local utility = self._utility
