-- Gitlab SDK RemoteMirror entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RemoteMirrorEntity = {}
RemoteMirrorEntity.__index = RemoteMirrorEntity


function RemoteMirrorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RemoteMirrorEntity)
  self._name = "remote_mirror"
  self._client = client
...
function RemoteMirrorEntity:get_name()
  return self._name
end


function RemoteMirrorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RemoteMirrorEntity.new(self._client, opts)
end
...
function RemoteMirrorEntity:mark_deleted()
  self._deleted = true
end


function RemoteMirrorEntity:deleted()
  return true == self._deleted
end


function RemoteMirrorEntity:data_set(args)
  if args ~= nil then
...
function RemoteMirrorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RemoteMirrorEntity:match_set(args)
  if args ~= nil then
...
function RemoteMirrorEntity:match_get()
...
function RemoteMirrorEntity:stream(action, args, callopts)
...
---@param reqmatch RemoteMirrorLoadMatch
---@param ctrl? table
---@return RemoteMirror
---@return string? err
function RemoteMirrorEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RemoteMirrorCreateData
---@param ctrl? table
---@return RemoteMirror
---@return string? err
function RemoteMirrorEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch RemoteMirrorRemoveMatch
---@param ctrl? table
---@return RemoteMirror
---@return string? err
function RemoteMirrorEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function RemoteMirrorEntity:_run_op(ctx, post_done)
  local utility = self._utility
