...
function NamespaceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NamespaceEntity)
...
function NamespaceEntity:get_name()
  return self._name
end


function NamespaceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NamespaceEntity.new(self._client, opts)
end
...
function NamespaceEntity:mark_deleted()
  self._deleted = true
end


function NamespaceEntity:deleted()
  return true == self._deleted
end


function NamespaceEntity:data_set(args)
  if args ~= nil then
...
function NamespaceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NamespaceEntity:match_set(args)
  if args ~= nil then
...
function NamespaceEntity:match_get()
...
function NamespaceEntity:stream(action, args, callopts)
...
---@param reqmatch NamespaceRemoveMatch
---@param ctrl? table
---@return Namespace
---@return string? err
function NamespaceEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function NamespaceEntity:_run_op(ctx, post_done)
  local utility = self._utility
