-- Gitlab SDK Conan entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ConanEntity = {}
ConanEntity.__index = ConanEntity


function ConanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConanEntity)
  self._name = "conan"
  self._client = client
...
function ConanEntity:get_name()
  return self._name
end


function ConanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConanEntity.new(self._client, opts)
end
...
function ConanEntity:mark_deleted()
  self._deleted = true
end


function ConanEntity:deleted()
  return true == self._deleted
end


function ConanEntity:data_set(args)
  if args ~= nil then
...
function ConanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConanEntity:match_set(args)
  if args ~= nil then
...
function ConanEntity:match_get()
...
function ConanEntity:stream(action, args, callopts)
...
---@param reqmatch ConanRemoveMatch
---@param ctrl? table
---@return Conan
---@return string? err
function ConanEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ConanEntity:_run_op(ctx, post_done)
  local utility = self._utility
