-- Gitlab SDK Badge entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local BadgeEntity = {}
BadgeEntity.__index = BadgeEntity


function BadgeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BadgeEntity)
  self._name = "badge"
  self._client = client
...
function BadgeEntity:get_name()
  return self._name
end


function BadgeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BadgeEntity.new(self._client, opts)
end
...
function BadgeEntity:mark_deleted()
  self._deleted = true
end


function BadgeEntity:deleted()
  return true == self._deleted
end


function BadgeEntity:data_set(args)
  if args ~= nil then
...
function BadgeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BadgeEntity:match_set(args)
  if args ~= nil then
...
function BadgeEntity:match_get()
...
function BadgeEntity:stream(action, args, callopts)
...
---@param reqmatch BadgeRemoveMatch
---@param ctrl? table
---@return Badge
---@return string? err
function BadgeEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function BadgeEntity:_run_op(ctx, post_done)
  local utility = self._utility
