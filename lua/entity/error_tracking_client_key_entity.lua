-- Gitlab SDK ErrorTrackingClientKey entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ErrorTrackingClientKeyEntity = {}
ErrorTrackingClientKeyEntity.__index = ErrorTrackingClientKeyEntity


function ErrorTrackingClientKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ErrorTrackingClientKeyEntity)
  self._name = "error_tracking_client_key"
  self._client = client
...
function ErrorTrackingClientKeyEntity:get_name()
  return self._name
end


function ErrorTrackingClientKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ErrorTrackingClientKeyEntity.new(self._client, opts)
end
...
function ErrorTrackingClientKeyEntity:mark_deleted()
  self._deleted = true
end


function ErrorTrackingClientKeyEntity:deleted()
  return true == self._deleted
end


function ErrorTrackingClientKeyEntity:data_set(args)
  if args ~= nil then
...
function ErrorTrackingClientKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ErrorTrackingClientKeyEntity:match_set(args)
  if args ~= nil then
...
function ErrorTrackingClientKeyEntity:match_get()
...
function ErrorTrackingClientKeyEntity:stream(action, args, callopts)
...
---@param reqmatch ErrorTrackingClientKeyRemoveMatch
---@param ctrl? table
---@return ErrorTrackingClientKey
---@return string? err
function ErrorTrackingClientKeyEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ErrorTrackingClientKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ErrorTrackingClientKeyEntity
