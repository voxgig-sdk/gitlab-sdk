...
local HookEntity = {}
HookEntity.__index = HookEntity


function HookEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HookEntity)
  self._name = "hook"
  self._client = client
...
function HookEntity:get_name()
  return self._name
end


function HookEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HookEntity.new(self._client, opts)
end
...
function HookEntity:mark_deleted()
  self._deleted = true
end


function HookEntity:deleted()
  return true == self._deleted
end


function HookEntity:data_set(args)
  if args ~= nil then
...
function HookEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HookEntity:match_set(args)
  if args ~= nil then
...
function HookEntity:stream(action, args, callopts)
...
---@param reqdata HookCreateData
---@param ctrl? table
---@return Hook
---@return string? err
function HookEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqdata HookUpdateData
---@param ctrl? table
---@return Hook
---@return string? err
function HookEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch HookRemoveMatch
---@param ctrl? table
---@return Hook
---@return string? err
function HookEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function HookEntity:_run_op(ctx, post_done)
  local utility = self._utility
