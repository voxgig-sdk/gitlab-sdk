-- Gitlab SDK DependencyProxy entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DependencyProxyEntity = {}
DependencyProxyEntity.__index = DependencyProxyEntity


function DependencyProxyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependencyProxyEntity)
  self._name = "dependency_proxy"
  self._client = client
...
function DependencyProxyEntity:get_name()
  return self._name
end


function DependencyProxyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependencyProxyEntity.new(self._client, opts)
end
...
function DependencyProxyEntity:mark_deleted()
  self._deleted = true
end


function DependencyProxyEntity:deleted()
  return true == self._deleted
end


function DependencyProxyEntity:data_set(args)
  if args ~= nil then
...
function DependencyProxyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependencyProxyEntity:match_set(args)
  if args ~= nil then
...
function DependencyProxyEntity:match_get()
...
function DependencyProxyEntity:stream(action, args, callopts)
...
---@param reqmatch DependencyProxyRemoveMatch
---@param ctrl? table
---@return DependencyProxy
---@return string? err
function DependencyProxyEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function DependencyProxyEntity:_run_op(ctx, post_done)
  local utility = self._utility
