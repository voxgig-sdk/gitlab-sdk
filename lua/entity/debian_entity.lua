-- Gitlab SDK Debian entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DebianEntity = {}
DebianEntity.__index = DebianEntity


function DebianEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DebianEntity)
  self._name = "debian"
  self._client = client
...
function DebianEntity:get_name()
  return self._name
end


function DebianEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DebianEntity.new(self._client, opts)
end
...
function DebianEntity:mark_deleted()
  self._deleted = true
end


function DebianEntity:deleted()
  return true == self._deleted
end


function DebianEntity:data_set(args)
  if args ~= nil then
...
function DebianEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DebianEntity:match_set(args)
  if args ~= nil then
...
function DebianEntity:match_get()
...
function DebianEntity:stream(action, args, callopts)
...
---@param reqdata DebianUpdateData
---@param ctrl? table
---@return Debian
---@return string? err
function DebianEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function DebianEntity:_run_op(ctx, post_done)
  local utility = self._utility
