-- Gitlab SDK FeatureFlagsUserList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FeatureFlagsUserListEntity = {}
FeatureFlagsUserListEntity.__index = FeatureFlagsUserListEntity


function FeatureFlagsUserListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FeatureFlagsUserListEntity)
  self._name = "feature_flags_user_list"
  self._client = client
...
function FeatureFlagsUserListEntity:get_name()
  return self._name
end


function FeatureFlagsUserListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FeatureFlagsUserListEntity.new(self._client, opts)
end
...
function FeatureFlagsUserListEntity:mark_deleted()
  self._deleted = true
end


function FeatureFlagsUserListEntity:deleted()
  return true == self._deleted
end


function FeatureFlagsUserListEntity:data_set(args)
  if args ~= nil then
...
function FeatureFlagsUserListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FeatureFlagsUserListEntity:match_set(args)
  if args ~= nil then
...
function FeatureFlagsUserListEntity:match_get()
...
function FeatureFlagsUserListEntity:stream(action, args, callopts)
...
---@param reqmatch FeatureFlagsUserListRemoveMatch
---@param ctrl? table
---@return FeatureFlagsUserList
---@return string? err
function FeatureFlagsUserListEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function FeatureFlagsUserListEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return FeatureFlagsUserListEntity
