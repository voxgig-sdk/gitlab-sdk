-- Gitlab SDK ProjectHook entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectHookEntity = {}
ProjectHookEntity.__index = ProjectHookEntity


function ProjectHookEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectHookEntity)
  self._name = "project_hook"
  self._client = client
...
function ProjectHookEntity:get_name()
  return self._name
end


function ProjectHookEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectHookEntity.new(self._client, opts)
end
...
function ProjectHookEntity:mark_deleted()
  self._deleted = true
end


function ProjectHookEntity:deleted()
  return true == self._deleted
end


function ProjectHookEntity:data_set(args)
  if args ~= nil then
...
function ProjectHookEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectHookEntity:match_set(args)
  if args ~= nil then
...
function ProjectHookEntity:match_get()
...
function ProjectHookEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectHookRemoveMatch
---@param ctrl? table
---@return ProjectHook
---@return string? err
function ProjectHookEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ProjectHookEntity:_run_op(ctx, post_done)
  local utility = self._utility
