-- Gitlab SDK Starrer entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local StarrerEntity = {}
StarrerEntity.__index = StarrerEntity


function StarrerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StarrerEntity)
  self._name = "starrer"
  self._client = client
...
function StarrerEntity:get_name()
  return self._name
end


function StarrerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StarrerEntity.new(self._client, opts)
end
...
function StarrerEntity:mark_deleted()
  self._deleted = true
end


function StarrerEntity:deleted()
  return true == self._deleted
end


function StarrerEntity:data_set(args)
  if args ~= nil then
...
function StarrerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StarrerEntity:match_set(args)
  if args ~= nil then
...
function StarrerEntity:match_get()
...
function StarrerEntity:stream(action, args, callopts)
...
---@param reqmatch StarrerListMatch
---@param ctrl? table
---@return Starrer[]
---@return string? err
function StarrerEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function StarrerEntity:_run_op(ctx, post_done)
  local utility = self._utility
