-- Gitlab SDK ProjectEntity entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectEntityEntity = {}
ProjectEntityEntity.__index = ProjectEntityEntity


function ProjectEntityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectEntityEntity)
  self._name = "project_entity"
  self._client = client
...
function ProjectEntityEntity:get_name()
  return self._name
end


function ProjectEntityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectEntityEntity.new(self._client, opts)
end
...
function ProjectEntityEntity:mark_deleted()
  self._deleted = true
end


function ProjectEntityEntity:deleted()
  return true == self._deleted
end


function ProjectEntityEntity:data_set(args)
  if args ~= nil then
...
function ProjectEntityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectEntityEntity:match_set(args)
  if args ~= nil then
...
function ProjectEntityEntity:match_get()
...
function ProjectEntityEntity:stream(action, args, callopts)
...
---@param reqdata ProjectEntityCreateData
---@param ctrl? table
---@return ProjectEntity
---@return string? err
function ProjectEntityEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ProjectEntityEntity:_run_op(ctx, post_done)
  local utility = self._utility
