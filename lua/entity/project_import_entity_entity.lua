-- Gitlab SDK ProjectImportEntity entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectImportEntityEntity = {}
ProjectImportEntityEntity.__index = ProjectImportEntityEntity


function ProjectImportEntityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectImportEntityEntity)
  self._name = "project_import_entity"
  self._client = client
...
function ProjectImportEntityEntity:get_name()
  return self._name
end


function ProjectImportEntityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectImportEntityEntity.new(self._client, opts)
end
...
function ProjectImportEntityEntity:mark_deleted()
  self._deleted = true
end


function ProjectImportEntityEntity:deleted()
  return true == self._deleted
end


function ProjectImportEntityEntity:data_set(args)
  if args ~= nil then
...
function ProjectImportEntityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectImportEntityEntity:match_set(args)
  if args ~= nil then
...
function ProjectImportEntityEntity:match_get()
...
function ProjectImportEntityEntity:stream(action, args, callopts)
...
---@param reqdata ProjectImportEntityCreateData
---@param ctrl? table
---@return ProjectImportEntity
---@return string? err
function ProjectImportEntityEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ProjectImportEntityEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ProjectImportEntityEntity
