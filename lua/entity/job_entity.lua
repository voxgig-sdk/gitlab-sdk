...
function JobEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JobEntity)
  self._name = "job"
  self._client = client
...
function JobEntity:get_name()
  return self._name
end


function JobEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JobEntity.new(self._client, opts)
end
...
function JobEntity:mark_deleted()
  self._deleted = true
end


function JobEntity:deleted()
  return true == self._deleted
end


function JobEntity:data_set(args)
  if args ~= nil then
...
function JobEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JobEntity:match_set(args)
  if args ~= nil then
...
function JobEntity:stream(action, args, callopts)
...
---@param reqmatch JobLoadMatch
---@param ctrl? table
---@return Job
---@return string? err
function JobEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata JobCreateData
---@param ctrl? table
---@return Job
---@return string? err
function JobEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata JobUpdateData
---@param ctrl? table
---@return Job
---@return string? err
function JobEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function JobEntity:_run_op(ctx, post_done)
  local utility = self._utility
