...
local SecureFileEntity = {}
SecureFileEntity.__index = SecureFileEntity


function SecureFileEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SecureFileEntity)
  self._name = "secure_file"
  self._client = client
...
function SecureFileEntity:get_name()
  return self._name
end


function SecureFileEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SecureFileEntity.new(self._client, opts)
end
...
function SecureFileEntity:mark_deleted()
  self._deleted = true
end


function SecureFileEntity:deleted()
  return true == self._deleted
end


function SecureFileEntity:data_set(args)
  if args ~= nil then
...
function SecureFileEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SecureFileEntity:match_set(args)
  if args ~= nil then
...
function SecureFileEntity:match_get()
...
function SecureFileEntity:stream(action, args, callopts)
...
---@param reqmatch SecureFileLoadMatch
---@param ctrl? table
---@return SecureFile
---@return string? err
function SecureFileEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SecureFileRemoveMatch
---@param ctrl? table
---@return SecureFile
---@return string? err
function SecureFileEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function SecureFileEntity:_run_op(ctx, post_done)
  local utility = self._utility
