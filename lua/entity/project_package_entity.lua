-- Gitlab SDK ProjectPackage entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectPackageEntity = {}
ProjectPackageEntity.__index = ProjectPackageEntity


function ProjectPackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectPackageEntity)
  self._name = "project_package"
  self._client = client
...
function ProjectPackageEntity:get_name()
  return self._name
end


function ProjectPackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectPackageEntity.new(self._client, opts)
end
...
function ProjectPackageEntity:mark_deleted()
  self._deleted = true
end


function ProjectPackageEntity:deleted()
  return true == self._deleted
end


function ProjectPackageEntity:data_set(args)
  if args ~= nil then
...
function ProjectPackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectPackageEntity:match_set(args)
  if args ~= nil then
...
function ProjectPackageEntity:match_get()
...
function ProjectPackageEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectPackageRemoveMatch
---@param ctrl? table
---@return ProjectPackage
---@return string? err
function ProjectPackageEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ProjectPackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
