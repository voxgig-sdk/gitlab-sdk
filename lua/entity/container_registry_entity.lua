-- Gitlab SDK ContainerRegistry entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ContainerRegistryEntity = {}
ContainerRegistryEntity.__index = ContainerRegistryEntity


function ContainerRegistryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContainerRegistryEntity)
  self._name = "container_registry"
  self._client = client
...
function ContainerRegistryEntity:get_name()
  return self._name
end


function ContainerRegistryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContainerRegistryEntity.new(self._client, opts)
end
...
function ContainerRegistryEntity:mark_deleted()
  self._deleted = true
end


function ContainerRegistryEntity:deleted()
  return true == self._deleted
end


function ContainerRegistryEntity:data_set(args)
  if args ~= nil then
...
function ContainerRegistryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContainerRegistryEntity:match_set(args)
  if args ~= nil then
...
function ContainerRegistryEntity:match_get()
...
function ContainerRegistryEntity:stream(action, args, callopts)
...
---@param reqmatch ContainerRegistryRemoveMatch
---@param ctrl? table
---@return ContainerRegistry
---@return string? err
function ContainerRegistryEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ContainerRegistryEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ContainerRegistryEntity
