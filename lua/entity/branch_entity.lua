-- Gitlab SDK Branch entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local BranchEntity = {}
BranchEntity.__index = BranchEntity


function BranchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BranchEntity)
  self._name = "branch"
  self._client = client
...
function BranchEntity:get_name()
  return self._name
end


function BranchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BranchEntity.new(self._client, opts)
end
...
function BranchEntity:mark_deleted()
  self._deleted = true
end


function BranchEntity:deleted()
  return true == self._deleted
end


function BranchEntity:data_set(args)
  if args ~= nil then
...
function BranchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BranchEntity:match_set(args)
  if args ~= nil then
...
function BranchEntity:match_get()
...
function BranchEntity:stream(action, args, callopts)
...
---@param reqmatch BranchRemoveMatch
---@param ctrl? table
---@return Branch
---@return string? err
function BranchEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function BranchEntity:_run_op(ctx, post_done)
  local utility = self._utility
