-- Gitlab SDK Rubygem entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RubygemEntity = {}
RubygemEntity.__index = RubygemEntity


function RubygemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RubygemEntity)
  self._name = "rubygem"
  self._client = client
...
function RubygemEntity:get_name()
  return self._name
end


function RubygemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RubygemEntity.new(self._client, opts)
end
...
function RubygemEntity:mark_deleted()
  self._deleted = true
end


function RubygemEntity:deleted()
  return true == self._deleted
end


function RubygemEntity:data_set(args)
  if args ~= nil then
...
function RubygemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RubygemEntity:match_set(args)
  if args ~= nil then
...
function RubygemEntity:match_get()
...
function RubygemEntity:stream(action, args, callopts)
...
---@param reqmatch RubygemLoadMatch
---@param ctrl? table
---@return Rubygem
---@return string? err
function RubygemEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function RubygemEntity:_run_op(ctx, post_done)
  local utility = self._utility
