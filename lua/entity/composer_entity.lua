-- Gitlab SDK Composer entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ComposerEntity = {}
ComposerEntity.__index = ComposerEntity


function ComposerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ComposerEntity)
  self._name = "composer"
  self._client = client
...
function ComposerEntity:get_name()
  return self._name
end


function ComposerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ComposerEntity.new(self._client, opts)
end
...
function ComposerEntity:mark_deleted()
  self._deleted = true
end


function ComposerEntity:deleted()
  return true == self._deleted
end


function ComposerEntity:data_set(args)
  if args ~= nil then
...
function ComposerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ComposerEntity:match_set(args)
  if args ~= nil then
...
function ComposerEntity:match_get()
...
function ComposerEntity:stream(action, args, callopts)
...
---@param reqdata ComposerCreateData
---@param ctrl? table
---@return Composer
---@return string? err
function ComposerEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ComposerEntity:_run_op(ctx, post_done)
  local utility = self._utility
