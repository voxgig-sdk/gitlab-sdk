...
local TerraformRegistryEntity = {}
TerraformRegistryEntity.__index = TerraformRegistryEntity


function TerraformRegistryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TerraformRegistryEntity)
  self._name = "terraform_registry"
  self._client = client
...
function TerraformRegistryEntity:get_name()
  return self._name
end


function TerraformRegistryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TerraformRegistryEntity.new(self._client, opts)
end
...
function TerraformRegistryEntity:mark_deleted()
  self._deleted = true
end


function TerraformRegistryEntity:deleted()
  return true == self._deleted
end


function TerraformRegistryEntity:data_set(args)
  if args ~= nil then
...
function TerraformRegistryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TerraformRegistryEntity:match_set(args)
  if args ~= nil then
...
function TerraformRegistryEntity:match_get()
...
function TerraformRegistryEntity:stream(action, args, callopts)
...
---@param reqmatch TerraformRegistryLoadMatch
---@param ctrl? table
---@return TerraformRegistry
---@return string? err
function TerraformRegistryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata TerraformRegistryUpdateData
---@param ctrl? table
---@return TerraformRegistry
---@return string? err
function TerraformRegistryEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function TerraformRegistryEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return TerraformRegistryEntity
