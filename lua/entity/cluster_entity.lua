-- Gitlab SDK Cluster entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ClusterEntity = {}
ClusterEntity.__index = ClusterEntity


function ClusterEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClusterEntity)
  self._name = "cluster"
  self._client = client
...
function ClusterEntity:get_name()
  return self._name
end


function ClusterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClusterEntity.new(self._client, opts)
end
...
function ClusterEntity:mark_deleted()
  self._deleted = true
end


function ClusterEntity:deleted()
  return true == self._deleted
end


function ClusterEntity:data_set(args)
  if args ~= nil then
...
function ClusterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClusterEntity:match_set(args)
  if args ~= nil then
...
function ClusterEntity:match_get()
...
function ClusterEntity:stream(action, args, callopts)
...
---@param reqmatch ClusterRemoveMatch
---@param ctrl? table
---@return Cluster
---@return string? err
function ClusterEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ClusterEntity:_run_op(ctx, post_done)
  local utility = self._utility
