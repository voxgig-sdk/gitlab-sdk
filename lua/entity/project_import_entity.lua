-- Gitlab SDK ProjectImport entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectImportEntityClient = {}
ProjectImportEntityClient.__index = ProjectImportEntityClient


function ProjectImportEntityClient.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectImportEntityClient)
  self._name = "project_import"
  self._client = client
...
function ProjectImportEntityClient:get_name()
  return self._name
end


function ProjectImportEntityClient:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectImportEntityClient.new(self._client, opts)
end
...
function ProjectImportEntityClient:mark_deleted()
  self._deleted = true
end


function ProjectImportEntityClient:deleted()
  return true == self._deleted
end


function ProjectImportEntityClient:data_set(args)
  if args ~= nil then
...
function ProjectImportEntityClient:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectImportEntityClient:match_set(args)
  if args ~= nil then
...
function ProjectImportEntityClient:match_get()
...
function ProjectImportEntityClient:stream(action, args, callopts)
...
---@param reqdata ProjectImportCreateData
---@param ctrl? table
---@return ProjectImport
---@return string? err
function ProjectImportEntityClient:create(reqdata, ctrl)
  local utility = self._utility
...
function ProjectImportEntityClient:_run_op(ctx, post_done)
  local utility = self._utility
...
return ProjectImportEntityClient
