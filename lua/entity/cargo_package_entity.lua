-- Gitlab SDK CargoPackage entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CargoPackageEntity = {}
CargoPackageEntity.__index = CargoPackageEntity


function CargoPackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CargoPackageEntity)
  self._name = "cargo_package"
  self._client = client
...
function CargoPackageEntity:get_name()
  return self._name
end


function CargoPackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CargoPackageEntity.new(self._client, opts)
end
...
function CargoPackageEntity:mark_deleted()
  self._deleted = true
end


function CargoPackageEntity:deleted()
  return true == self._deleted
end


function CargoPackageEntity:data_set(args)
  if args ~= nil then
...
function CargoPackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CargoPackageEntity:match_set(args)
  if args ~= nil then
...
function CargoPackageEntity:match_get()
...
function CargoPackageEntity:stream(action, args, callopts)
...
---@param reqmatch CargoPackageLoadMatch
---@param ctrl? table
---@return CargoPackage
---@return string? err
function CargoPackageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CargoPackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
