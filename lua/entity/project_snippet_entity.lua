-- Gitlab SDK ProjectSnippet entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectSnippetEntity = {}
ProjectSnippetEntity.__index = ProjectSnippetEntity


function ProjectSnippetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectSnippetEntity)
  self._name = "project_snippet"
  self._client = client
...
function ProjectSnippetEntity:get_name()
  return self._name
end


function ProjectSnippetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectSnippetEntity.new(self._client, opts)
end
...
function ProjectSnippetEntity:mark_deleted()
  self._deleted = true
end


function ProjectSnippetEntity:deleted()
  return true == self._deleted
end


function ProjectSnippetEntity:data_set(args)
  if args ~= nil then
...
function ProjectSnippetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectSnippetEntity:match_set(args)
  if args ~= nil then
...
function ProjectSnippetEntity:match_get()
...
function ProjectSnippetEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectSnippetRemoveMatch
---@param ctrl? table
---@return ProjectSnippet
---@return string? err
function ProjectSnippetEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ProjectSnippetEntity:_run_op(ctx, post_done)
  local utility = self._utility
