...
local PackageFileEntity = {}
PackageFileEntity.__index = PackageFileEntity


function PackageFileEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PackageFileEntity)
  self._name = "package_file"
  self._client = client
...
function PackageFileEntity:get_name()
  return self._name
end


function PackageFileEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PackageFileEntity.new(self._client, opts)
end
...
function PackageFileEntity:mark_deleted()
  self._deleted = true
end


function PackageFileEntity:deleted()
  return true == self._deleted
end


function PackageFileEntity:data_set(args)
  if args ~= nil then
...
function PackageFileEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PackageFileEntity:match_set(args)
  if args ~= nil then
...
function PackageFileEntity:match_get()
...
function PackageFileEntity:stream(action, args, callopts)
...
---@param reqmatch PackageFileLoadMatch
---@param ctrl? table
---@return PackageFile
---@return string? err
function PackageFileEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PackageFileRemoveMatch
---@param ctrl? table
---@return PackageFile
---@return string? err
function PackageFileEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function PackageFileEntity:_run_op(ctx, post_done)
  local utility = self._utility
