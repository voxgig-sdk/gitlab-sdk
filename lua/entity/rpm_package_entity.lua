...
local RpmPackageEntity = {}
RpmPackageEntity.__index = RpmPackageEntity


function RpmPackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RpmPackageEntity)
  self._name = "rpm_package"
  self._client = client
...
function RpmPackageEntity:get_name()
  return self._name
end


function RpmPackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RpmPackageEntity.new(self._client, opts)
end
...
function RpmPackageEntity:mark_deleted()
  self._deleted = true
end


function RpmPackageEntity:deleted()
  return true == self._deleted
end


function RpmPackageEntity:data_set(args)
  if args ~= nil then
...
function RpmPackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RpmPackageEntity:match_set(args)
  if args ~= nil then
...
function RpmPackageEntity:match_get()
...
function RpmPackageEntity:stream(action, args, callopts)
...
---@param reqmatch RpmPackageLoadMatch
---@param ctrl? table
---@return RpmPackage
---@return string? err
function RpmPackageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RpmPackageCreateData
---@param ctrl? table
---@return RpmPackage
---@return string? err
function RpmPackageEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function RpmPackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
