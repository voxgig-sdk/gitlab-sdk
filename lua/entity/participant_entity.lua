-- Gitlab SDK Participant entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ParticipantEntity = {}
ParticipantEntity.__index = ParticipantEntity


function ParticipantEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ParticipantEntity)
  self._name = "participant"
  self._client = client
...
function ParticipantEntity:get_name()
  return self._name
end


function ParticipantEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ParticipantEntity.new(self._client, opts)
end
...
function ParticipantEntity:mark_deleted()
  self._deleted = true
end


function ParticipantEntity:deleted()
  return true == self._deleted
end


function ParticipantEntity:data_set(args)
  if args ~= nil then
...
function ParticipantEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ParticipantEntity:match_set(args)
  if args ~= nil then
...
function ParticipantEntity:match_get()
...
function ParticipantEntity:stream(action, args, callopts)
...
---@param reqmatch ParticipantListMatch
---@param ctrl? table
---@return Participant[]
---@return string? err
function ParticipantEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ParticipantEntity:_run_op(ctx, post_done)
  local utility = self._utility
