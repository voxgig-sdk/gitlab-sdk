-- Gitlab SDK TestReport entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TestReportEntity = {}
TestReportEntity.__index = TestReportEntity


function TestReportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TestReportEntity)
  self._name = "test_report"
  self._client = client
...
function TestReportEntity:get_name()
  return self._name
end


function TestReportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TestReportEntity.new(self._client, opts)
end
...
function TestReportEntity:mark_deleted()
  self._deleted = true
end


function TestReportEntity:deleted()
  return true == self._deleted
end


function TestReportEntity:data_set(args)
  if args ~= nil then
...
function TestReportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TestReportEntity:match_set(args)
  if args ~= nil then
...
function TestReportEntity:match_get()
...
function TestReportEntity:stream(action, args, callopts)
...
---@param reqmatch TestReportListMatch
---@param ctrl? table
---@return TestReport[]
---@return string? err
function TestReportEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TestReportEntity:_run_op(ctx, post_done)
  local utility = self._utility
