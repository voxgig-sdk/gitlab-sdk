...
local ConanPackageEntity = {}
ConanPackageEntity.__index = ConanPackageEntity


function ConanPackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConanPackageEntity)
  self._name = "conan_package"
  self._client = client
...
function ConanPackageEntity:get_name()
  return self._name
end


function ConanPackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConanPackageEntity.new(self._client, opts)
end
...
function ConanPackageEntity:mark_deleted()
  self._deleted = true
end


function ConanPackageEntity:deleted()
  return true == self._deleted
end


function ConanPackageEntity:data_set(args)
  if args ~= nil then
...
function ConanPackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConanPackageEntity:match_set(args)
  if args ~= nil then
...
function ConanPackageEntity:match_get()
...
function ConanPackageEntity:stream(action, args, callopts)
...
---@param reqmatch ConanPackageLoadMatch
---@param ctrl? table
---@return ConanPackage
---@return string? err
function ConanPackageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ConanPackageUpdateData
---@param ctrl? table
---@return ConanPackage
---@return string? err
function ConanPackageEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch ConanPackageRemoveMatch
---@param ctrl? table
---@return ConanPackage
---@return string? err
function ConanPackageEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function ConanPackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
