-- Gitlab SDK Wiki entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WikiEntity = {}
WikiEntity.__index = WikiEntity


function WikiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WikiEntity)
  self._name = "wiki"
  self._client = client
...
function WikiEntity:get_name()
  return self._name
end


function WikiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WikiEntity.new(self._client, opts)
end
...
function WikiEntity:mark_deleted()
  self._deleted = true
end


function WikiEntity:deleted()
  return true == self._deleted
end


function WikiEntity:data_set(args)
  if args ~= nil then
...
function WikiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WikiEntity:match_set(args)
  if args ~= nil then
...
function WikiEntity:stream(action, args, callopts)
...
---@param reqmatch WikiRemoveMatch
---@param ctrl? table
---@return Wiki
---@return string? err
function WikiEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function WikiEntity:_run_op(ctx, post_done)
  local utility = self._utility
