-- Gitlab SDK IssueLink entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local IssueLinkEntity = {}
IssueLinkEntity.__index = IssueLinkEntity


function IssueLinkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IssueLinkEntity)
  self._name = "issue_link"
  self._client = client
...
function IssueLinkEntity:get_name()
  return self._name
end


function IssueLinkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IssueLinkEntity.new(self._client, opts)
end
...
function IssueLinkEntity:mark_deleted()
  self._deleted = true
end


function IssueLinkEntity:deleted()
  return true == self._deleted
end


function IssueLinkEntity:data_set(args)
  if args ~= nil then
...
function IssueLinkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IssueLinkEntity:match_set(args)
  if args ~= nil then
...
function IssueLinkEntity:match_get()
...
function IssueLinkEntity:stream(action, args, callopts)
...
---@param reqmatch IssueLinkRemoveMatch
---@param ctrl? table
---@return IssueLink
---@return string? err
function IssueLinkEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function IssueLinkEntity:_run_op(ctx, post_done)
  local utility = self._utility
