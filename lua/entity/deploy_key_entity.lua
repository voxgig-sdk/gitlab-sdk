-- Gitlab SDK DeployKey entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DeployKeyEntity = {}
DeployKeyEntity.__index = DeployKeyEntity


function DeployKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeployKeyEntity)
  self._name = "deploy_key"
  self._client = client
...
function DeployKeyEntity:get_name()
  return self._name
end


function DeployKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeployKeyEntity.new(self._client, opts)
end
...
function DeployKeyEntity:mark_deleted()
  self._deleted = true
end


function DeployKeyEntity:deleted()
  return true == self._deleted
end


function DeployKeyEntity:data_set(args)
  if args ~= nil then
...
function DeployKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeployKeyEntity:match_set(args)
  if args ~= nil then
...
function DeployKeyEntity:match_get()
...
function DeployKeyEntity:stream(action, args, callopts)
...
---@param reqmatch DeployKeyRemoveMatch
---@param ctrl? table
---@return DeployKey
---@return string? err
function DeployKeyEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function DeployKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
