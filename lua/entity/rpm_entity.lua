-- Gitlab SDK Rpm entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RpmEntity = {}
RpmEntity.__index = RpmEntity


function RpmEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RpmEntity)
  self._name = "rpm"
  self._client = client
...
function RpmEntity:get_name()
  return self._name
end


function RpmEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RpmEntity.new(self._client, opts)
end
...
function RpmEntity:mark_deleted()
  self._deleted = true
end


function RpmEntity:deleted()
  return true == self._deleted
end


function RpmEntity:data_set(args)
  if args ~= nil then
...
function RpmEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RpmEntity:match_set(args)
  if args ~= nil then
...
function RpmEntity:stream(action, args, callopts)
...
---@param reqdata RpmCreateData
---@param ctrl? table
---@return Rpm
---@return string? err
function RpmEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RpmEntity:_run_op(ctx, post_done)
  local utility = self._utility
