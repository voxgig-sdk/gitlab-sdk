...
local RunnerEntity = {}
RunnerEntity.__index = RunnerEntity


function RunnerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RunnerEntity)
  self._name = "runner"
  self._client = client
...
function RunnerEntity:get_name()
  return self._name
end


function RunnerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RunnerEntity.new(self._client, opts)
end
...
function RunnerEntity:mark_deleted()
  self._deleted = true
end


function RunnerEntity:deleted()
  return true == self._deleted
end


function RunnerEntity:data_set(args)
  if args ~= nil then
...
function RunnerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RunnerEntity:match_set(args)
  if args ~= nil then
...
function RunnerEntity:match_get()
...
function RunnerEntity:stream(action, args, callopts)
...
---@param reqdata RunnerCreateData
---@param ctrl? table
---@return Runner
---@return string? err
function RunnerEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch RunnerRemoveMatch
---@param ctrl? table
---@return Runner
---@return string? err
function RunnerEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function RunnerEntity:_run_op(ctx, post_done)
  local utility = self._utility
