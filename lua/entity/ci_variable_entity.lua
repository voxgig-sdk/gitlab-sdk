-- Gitlab SDK CiVariable entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CiVariableEntity = {}
CiVariableEntity.__index = CiVariableEntity


function CiVariableEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CiVariableEntity)
  self._name = "ci_variable"
  self._client = client
...
function CiVariableEntity:get_name()
  return self._name
end


function CiVariableEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CiVariableEntity.new(self._client, opts)
end
...
function CiVariableEntity:mark_deleted()
  self._deleted = true
end


function CiVariableEntity:deleted()
  return true == self._deleted
end


function CiVariableEntity:data_set(args)
  if args ~= nil then
...
function CiVariableEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CiVariableEntity:match_set(args)
  if args ~= nil then
...
function CiVariableEntity:match_get()
...
function CiVariableEntity:stream(action, args, callopts)
...
---@param reqmatch CiVariableRemoveMatch
---@param ctrl? table
---@return CiVariable
---@return string? err
function CiVariableEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function CiVariableEntity:_run_op(ctx, post_done)
  local utility = self._utility
