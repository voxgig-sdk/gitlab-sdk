-- Gitlab SDK PersonalAccessToken entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PersonalAccessTokenEntity = {}
PersonalAccessTokenEntity.__index = PersonalAccessTokenEntity


function PersonalAccessTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PersonalAccessTokenEntity)
  self._name = "personal_access_token"
  self._client = client
...
function PersonalAccessTokenEntity:get_name()
  return self._name
end


function PersonalAccessTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PersonalAccessTokenEntity.new(self._client, opts)
end
...
function PersonalAccessTokenEntity:mark_deleted()
  self._deleted = true
end


function PersonalAccessTokenEntity:deleted()
  return true == self._deleted
end


function PersonalAccessTokenEntity:data_set(args)
  if args ~= nil then
...
function PersonalAccessTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PersonalAccessTokenEntity:match_set(args)
  if args ~= nil then
...
function PersonalAccessTokenEntity:match_get()
...
function PersonalAccessTokenEntity:stream(action, args, callopts)
...
---@param reqmatch PersonalAccessTokenRemoveMatch
---@param ctrl? table
---@return PersonalAccessToken
---@return string? err
function PersonalAccessTokenEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function PersonalAccessTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
