-- Gitlab SDK ReleaseLink entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReleaseLinkEntity = {}
ReleaseLinkEntity.__index = ReleaseLinkEntity


function ReleaseLinkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReleaseLinkEntity)
  self._name = "release_link"
  self._client = client
...
function ReleaseLinkEntity:get_name()
  return self._name
end


function ReleaseLinkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReleaseLinkEntity.new(self._client, opts)
end
...
function ReleaseLinkEntity:mark_deleted()
  self._deleted = true
end


function ReleaseLinkEntity:deleted()
  return true == self._deleted
end


function ReleaseLinkEntity:data_set(args)
  if args ~= nil then
...
function ReleaseLinkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReleaseLinkEntity:match_set(args)
  if args ~= nil then
...
function ReleaseLinkEntity:match_get()
...
function ReleaseLinkEntity:stream(action, args, callopts)
...
---@param reqmatch ReleaseLinkRemoveMatch
---@param ctrl? table
---@return ReleaseLink
---@return string? err
function ReleaseLinkEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ReleaseLinkEntity:_run_op(ctx, post_done)
  local utility = self._utility
