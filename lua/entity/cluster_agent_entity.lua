-- Gitlab SDK ClusterAgent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ClusterAgentEntity = {}
ClusterAgentEntity.__index = ClusterAgentEntity


function ClusterAgentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClusterAgentEntity)
  self._name = "cluster_agent"
  self._client = client
...
function ClusterAgentEntity:get_name()
  return self._name
end


function ClusterAgentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClusterAgentEntity.new(self._client, opts)
end
...
function ClusterAgentEntity:mark_deleted()
  self._deleted = true
end


function ClusterAgentEntity:deleted()
  return true == self._deleted
end


function ClusterAgentEntity:data_set(args)
  if args ~= nil then
...
function ClusterAgentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClusterAgentEntity:match_set(args)
  if args ~= nil then
...
function ClusterAgentEntity:match_get()
...
function ClusterAgentEntity:stream(action, args, callopts)
...
---@param reqmatch ClusterAgentRemoveMatch
---@param ctrl? table
---@return ClusterAgent
---@return string? err
function ClusterAgentEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ClusterAgentEntity:_run_op(ctx, post_done)
  local utility = self._utility
