...
local TerraformStateEntity = {}
TerraformStateEntity.__index = TerraformStateEntity


function TerraformStateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TerraformStateEntity)
  self._name = "terraform_state"
  self._client = client
...
function TerraformStateEntity:get_name()
  return self._name
end


function TerraformStateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TerraformStateEntity.new(self._client, opts)
end
...
function TerraformStateEntity:mark_deleted()
  self._deleted = true
end


function TerraformStateEntity:deleted()
  return true == self._deleted
end


function TerraformStateEntity:data_set(args)
  if args ~= nil then
...
function TerraformStateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TerraformStateEntity:match_set(args)
  if args ~= nil then
...
function TerraformStateEntity:match_get()
...
function TerraformStateEntity:stream(action, args, callopts)
...
---@param reqmatch TerraformStateLoadMatch
---@param ctrl? table
---@return TerraformState
---@return string? err
function TerraformStateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata TerraformStateCreateData
---@param ctrl? table
---@return TerraformState
---@return string? err
function TerraformStateEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch TerraformStateRemoveMatch
---@param ctrl? table
---@return TerraformState
---@return string? err
function TerraformStateEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function TerraformStateEntity:_run_op(ctx, post_done)
  local utility = self._utility
