-- Gitlab SDK DebianPackage entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DebianPackageEntity = {}
DebianPackageEntity.__index = DebianPackageEntity


function DebianPackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DebianPackageEntity)
  self._name = "debian_package"
  self._client = client
...
function DebianPackageEntity:get_name()
  return self._name
end


function DebianPackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DebianPackageEntity.new(self._client, opts)
end
...
function DebianPackageEntity:mark_deleted()
  self._deleted = true
end


function DebianPackageEntity:deleted()
  return true == self._deleted
end


function DebianPackageEntity:data_set(args)
  if args ~= nil then
...
function DebianPackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DebianPackageEntity:match_set(args)
  if args ~= nil then
...
function DebianPackageEntity:match_get()
...
function DebianPackageEntity:stream(action, args, callopts)
...
---@param reqmatch DebianPackageLoadMatch
---@param ctrl? table
---@return DebianPackage
---@return string? err
function DebianPackageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DebianPackageUpdateData
---@param ctrl? table
---@return DebianPackage
---@return string? err
function DebianPackageEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function DebianPackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
