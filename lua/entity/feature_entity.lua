...
function FeatureEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FeatureEntity)
  self._name = "feature"
  self._client = client
...
function FeatureEntity:get_name()
  return self._name
end


function FeatureEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FeatureEntity.new(self._client, opts)
end
...
function FeatureEntity:mark_deleted()
  self._deleted = true
end


function FeatureEntity:deleted()
  return true == self._deleted
end


function FeatureEntity:data_set(args)
  if args ~= nil then
...
function FeatureEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FeatureEntity:match_set(args)
  if args ~= nil then
...
function FeatureEntity:match_get()
...
function FeatureEntity:stream(action, args, callopts)
...
---@param reqmatch FeatureRemoveMatch
---@param ctrl? table
---@return Feature
---@return string? err
function FeatureEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function FeatureEntity:_run_op(ctx, post_done)
  local utility = self._utility
