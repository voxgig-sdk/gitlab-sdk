-- Gitlab SDK DeployToken entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DeployTokenEntity = {}
DeployTokenEntity.__index = DeployTokenEntity


function DeployTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeployTokenEntity)
  self._name = "deploy_token"
  self._client = client
...
function DeployTokenEntity:get_name()
  return self._name
end


function DeployTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeployTokenEntity.new(self._client, opts)
end
...
function DeployTokenEntity:mark_deleted()
  self._deleted = true
end


function DeployTokenEntity:deleted()
  return true == self._deleted
end


function DeployTokenEntity:data_set(args)
  if args ~= nil then
...
function DeployTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeployTokenEntity:match_set(args)
  if args ~= nil then
...
function DeployTokenEntity:match_get()
...
function DeployTokenEntity:stream(action, args, callopts)
...
---@param reqmatch DeployTokenRemoveMatch
---@param ctrl? table
---@return DeployToken
---@return string? err
function DeployTokenEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function DeployTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
