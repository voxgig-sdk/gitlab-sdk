-- Gitlab SDK Invitation entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InvitationEntity = {}
InvitationEntity.__index = InvitationEntity


function InvitationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InvitationEntity)
  self._name = "invitation"
  self._client = client
...
function InvitationEntity:get_name()
  return self._name
end


function InvitationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InvitationEntity.new(self._client, opts)
end
...
function InvitationEntity:mark_deleted()
  self._deleted = true
end


function InvitationEntity:deleted()
  return true == self._deleted
end


function InvitationEntity:data_set(args)
  if args ~= nil then
...
function InvitationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InvitationEntity:match_set(args)
  if args ~= nil then
...
function InvitationEntity:match_get()
...
function InvitationEntity:stream(action, args, callopts)
...
---@param reqmatch InvitationRemoveMatch
---@param ctrl? table
---@return Invitation
---@return string? err
function InvitationEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function InvitationEntity:_run_op(ctx, post_done)
  local utility = self._utility
