-- Gitlab SDK Slack entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SlackEntity = {}
SlackEntity.__index = SlackEntity


function SlackEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SlackEntity)
  self._name = "slack"
  self._client = client
...
function SlackEntity:get_name()
  return self._name
end


function SlackEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SlackEntity.new(self._client, opts)
end
...
function SlackEntity:mark_deleted()
  self._deleted = true
end


function SlackEntity:deleted()
  return true == self._deleted
end


function SlackEntity:data_set(args)
  if args ~= nil then
...
function SlackEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SlackEntity:match_set(args)
  if args ~= nil then
...
function SlackEntity:match_get()
...
function SlackEntity:stream(action, args, callopts)
...
---@param reqdata SlackCreateData
---@param ctrl? table
---@return Slack
---@return string? err
function SlackEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function SlackEntity:_run_op(ctx, post_done)
  local utility = self._utility
