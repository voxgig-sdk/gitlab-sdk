...
local HelmPackageEntity = {}
HelmPackageEntity.__index = HelmPackageEntity


function HelmPackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HelmPackageEntity)
  self._name = "helm_package"
  self._client = client
...
function HelmPackageEntity:get_name()
  return self._name
end


function HelmPackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HelmPackageEntity.new(self._client, opts)
end
...
function HelmPackageEntity:mark_deleted()
  self._deleted = true
end


function HelmPackageEntity:deleted()
  return true == self._deleted
end


function HelmPackageEntity:data_set(args)
  if args ~= nil then
...
function HelmPackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HelmPackageEntity:match_set(args)
  if args ~= nil then
...
function HelmPackageEntity:match_get()
...
function HelmPackageEntity:stream(action, args, callopts)
...
---@param reqmatch HelmPackageLoadMatch
---@param ctrl? table
---@return HelmPackage
---@return string? err
function HelmPackageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata HelmPackageCreateData
---@param ctrl? table
---@return HelmPackage
---@return string? err
function HelmPackageEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function HelmPackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
