-- Gitlab SDK Nuget entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local NugetEntity = {}
NugetEntity.__index = NugetEntity


function NugetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NugetEntity)
  self._name = "nuget"
  self._client = client
...
function NugetEntity:get_name()
  return self._name
end


function NugetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NugetEntity.new(self._client, opts)
end
...
function NugetEntity:mark_deleted()
  self._deleted = true
end


function NugetEntity:deleted()
  return true == self._deleted
end


function NugetEntity:data_set(args)
  if args ~= nil then
...
function NugetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NugetEntity:match_set(args)
  if args ~= nil then
...
function NugetEntity:match_get()
...
function NugetEntity:stream(action, args, callopts)
...
---@param reqdata NugetUpdateData
---@param ctrl? table
---@return Nuget
---@return string? err
function NugetEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function NugetEntity:_run_op(ctx, post_done)
  local utility = self._utility
