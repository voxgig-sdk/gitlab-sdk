-- Gitlab SDK GoProxy entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GoProxyEntity = {}
GoProxyEntity.__index = GoProxyEntity


function GoProxyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GoProxyEntity)
  self._name = "go_proxy"
  self._client = client
...
function GoProxyEntity:get_name()
  return self._name
end


function GoProxyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GoProxyEntity.new(self._client, opts)
end
...
function GoProxyEntity:mark_deleted()
  self._deleted = true
end


function GoProxyEntity:deleted()
  return true == self._deleted
end


function GoProxyEntity:data_set(args)
  if args ~= nil then
...
function GoProxyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GoProxyEntity:match_set(args)
  if args ~= nil then
...
function GoProxyEntity:match_get()
...
function GoProxyEntity:stream(action, args, callopts)
...
---@param reqmatch GoProxyLoadMatch
---@param ctrl? table
---@return GoProxy
---@return string? err
function GoProxyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GoProxyEntity:_run_op(ctx, post_done)
  local utility = self._utility
