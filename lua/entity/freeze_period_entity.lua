-- Gitlab SDK FreezePeriod entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FreezePeriodEntity = {}
FreezePeriodEntity.__index = FreezePeriodEntity


function FreezePeriodEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FreezePeriodEntity)
  self._name = "freeze_period"
  self._client = client
...
function FreezePeriodEntity:get_name()
  return self._name
end


function FreezePeriodEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FreezePeriodEntity.new(self._client, opts)
end
...
function FreezePeriodEntity:mark_deleted()
  self._deleted = true
end


function FreezePeriodEntity:deleted()
  return true == self._deleted
end


function FreezePeriodEntity:data_set(args)
  if args ~= nil then
...
function FreezePeriodEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FreezePeriodEntity:match_set(args)
  if args ~= nil then
...
function FreezePeriodEntity:match_get()
...
function FreezePeriodEntity:stream(action, args, callopts)
...
---@param reqmatch FreezePeriodRemoveMatch
---@param ctrl? table
---@return FreezePeriod
---@return string? err
function FreezePeriodEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function FreezePeriodEntity:_run_op(ctx, post_done)
  local utility = self._utility
