-- Gitlab SDK WebCommit entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WebCommitEntity = {}
WebCommitEntity.__index = WebCommitEntity


function WebCommitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebCommitEntity)
  self._name = "web_commit"
  self._client = client
...
function WebCommitEntity:get_name()
  return self._name
end


function WebCommitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebCommitEntity.new(self._client, opts)
end
...
function WebCommitEntity:mark_deleted()
  self._deleted = true
end


function WebCommitEntity:deleted()
  return true == self._deleted
end


function WebCommitEntity:data_set(args)
  if args ~= nil then
...
function WebCommitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebCommitEntity:match_set(args)
  if args ~= nil then
...
function WebCommitEntity:match_get()
...
function WebCommitEntity:stream(action, args, callopts)
...
---@param reqmatch WebCommitLoadMatch
---@param ctrl? table
---@return WebCommit
---@return string? err
function WebCommitEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WebCommitEntity:_run_op(ctx, post_done)
  local utility = self._utility
