...
local PypiPackageEntity = {}
PypiPackageEntity.__index = PypiPackageEntity


function PypiPackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PypiPackageEntity)
  self._name = "pypi_package"
  self._client = client
...
function PypiPackageEntity:get_name()
  return self._name
end


function PypiPackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PypiPackageEntity.new(self._client, opts)
end
...
function PypiPackageEntity:mark_deleted()
  self._deleted = true
end


function PypiPackageEntity:deleted()
  return true == self._deleted
end


function PypiPackageEntity:data_set(args)
  if args ~= nil then
...
function PypiPackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PypiPackageEntity:match_set(args)
  if args ~= nil then
...
function PypiPackageEntity:match_get()
...
function PypiPackageEntity:stream(action, args, callopts)
...
---@param reqmatch PypiPackageLoadMatch
---@param ctrl? table
---@return PypiPackage
---@return string? err
function PypiPackageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata PypiPackageCreateData
---@param ctrl? table
---@return PypiPackage
---@return string? err
function PypiPackageEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function PypiPackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
