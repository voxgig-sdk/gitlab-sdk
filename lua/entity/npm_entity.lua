-- Gitlab SDK Npm entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local NpmEntity = {}
NpmEntity.__index = NpmEntity


function NpmEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NpmEntity)
  self._name = "npm"
  self._client = client
...
function NpmEntity:get_name()
  return self._name
end


function NpmEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NpmEntity.new(self._client, opts)
end
...
function NpmEntity:mark_deleted()
  self._deleted = true
end


function NpmEntity:deleted()
  return true == self._deleted
end


function NpmEntity:data_set(args)
  if args ~= nil then
...
function NpmEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NpmEntity:match_set(args)
  if args ~= nil then
...
function NpmEntity:stream(action, args, callopts)
...
---@param reqdata NpmUpdateData
---@param ctrl? table
---@return Npm
---@return string? err
function NpmEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function NpmEntity:_run_op(ctx, post_done)
  local utility = self._utility
