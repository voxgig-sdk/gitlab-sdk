-- Gitlab SDK Deployment entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DeploymentEntity = {}
DeploymentEntity.__index = DeploymentEntity


function DeploymentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeploymentEntity)
...
function DeploymentEntity:get_name()
  return self._name
end


function DeploymentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeploymentEntity.new(self._client, opts)
end
...
function DeploymentEntity:mark_deleted()
  self._deleted = true
end


function DeploymentEntity:deleted()
  return true == self._deleted
end


function DeploymentEntity:data_set(args)
  if args ~= nil then
...
function DeploymentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeploymentEntity:match_set(args)
  if args ~= nil then
...
function DeploymentEntity:match_get()
...
function DeploymentEntity:stream(action, args, callopts)
...
---@param reqmatch DeploymentRemoveMatch
---@param ctrl? table
---@return Deployment
---@return string? err
function DeploymentEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function DeploymentEntity:_run_op(ctx, post_done)
  local utility = self._utility
