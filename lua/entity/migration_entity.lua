-- Gitlab SDK Migration entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MigrationEntity = {}
MigrationEntity.__index = MigrationEntity


function MigrationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MigrationEntity)
  self._name = "migration"
  self._client = client
...
function MigrationEntity:get_name()
  return self._name
end


function MigrationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MigrationEntity.new(self._client, opts)
end
...
function MigrationEntity:mark_deleted()
  self._deleted = true
end


function MigrationEntity:deleted()
  return true == self._deleted
end


function MigrationEntity:data_set(args)
  if args ~= nil then
...
function MigrationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MigrationEntity:match_set(args)
  if args ~= nil then
...
function MigrationEntity:match_get()
...
function MigrationEntity:stream(action, args, callopts)
...
---@param reqdata MigrationCreateData
---@param ctrl? table
---@return Migration
---@return string? err
function MigrationEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function MigrationEntity:_run_op(ctx, post_done)
  local utility = self._utility
