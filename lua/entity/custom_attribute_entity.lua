-- Gitlab SDK CustomAttribute entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CustomAttributeEntity = {}
CustomAttributeEntity.__index = CustomAttributeEntity


function CustomAttributeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomAttributeEntity)
  self._name = "custom_attribute"
  self._client = client
...
function CustomAttributeEntity:get_name()
  return self._name
end


function CustomAttributeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomAttributeEntity.new(self._client, opts)
end
...
function CustomAttributeEntity:mark_deleted()
  self._deleted = true
end


function CustomAttributeEntity:deleted()
  return true == self._deleted
end


function CustomAttributeEntity:data_set(args)
  if args ~= nil then
...
function CustomAttributeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomAttributeEntity:match_set(args)
  if args ~= nil then
...
function CustomAttributeEntity:match_get()
...
function CustomAttributeEntity:stream(action, args, callopts)
...
---@param reqmatch CustomAttributeLoadMatch
---@param ctrl? table
---@return CustomAttribute
---@return string? err
function CustomAttributeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CustomAttributeEntity:_run_op(ctx, post_done)
  local utility = self._utility
