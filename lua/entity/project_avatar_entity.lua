-- Gitlab SDK ProjectAvatar entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProjectAvatarEntity = {}
ProjectAvatarEntity.__index = ProjectAvatarEntity


function ProjectAvatarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProjectAvatarEntity)
  self._name = "project_avatar"
  self._client = client
...
function ProjectAvatarEntity:get_name()
  return self._name
end


function ProjectAvatarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProjectAvatarEntity.new(self._client, opts)
end
...
function ProjectAvatarEntity:mark_deleted()
  self._deleted = true
end


function ProjectAvatarEntity:deleted()
  return true == self._deleted
end


function ProjectAvatarEntity:data_set(args)
  if args ~= nil then
...
function ProjectAvatarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProjectAvatarEntity:match_set(args)
  if args ~= nil then
...
function ProjectAvatarEntity:match_get()
...
function ProjectAvatarEntity:stream(action, args, callopts)
...
---@param reqmatch ProjectAvatarLoadMatch
---@param ctrl? table
---@return ProjectAvatar
---@return string? err
function ProjectAvatarEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ProjectAvatarEntity:_run_op(ctx, post_done)
  local utility = self._utility
