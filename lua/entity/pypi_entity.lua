-- Gitlab SDK Pypi entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PypiEntity = {}
PypiEntity.__index = PypiEntity


function PypiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PypiEntity)
  self._name = "pypi"
  self._client = client
...
function PypiEntity:get_name()
  return self._name
end


function PypiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PypiEntity.new(self._client, opts)
end
...
function PypiEntity:mark_deleted()
  self._deleted = true
end


function PypiEntity:deleted()
  return true == self._deleted
end


function PypiEntity:data_set(args)
  if args ~= nil then
...
function PypiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PypiEntity:match_set(args)
  if args ~= nil then
...
function PypiEntity:stream(action, args, callopts)
...
---@param reqdata PypiCreateData
---@param ctrl? table
---@return Pypi
---@return string? err
function PypiEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PypiEntity:_run_op(ctx, post_done)
  local utility = self._utility
