-- Gitlab SDK DebianDistribution entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DebianDistributionEntity = {}
DebianDistributionEntity.__index = DebianDistributionEntity


function DebianDistributionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DebianDistributionEntity)
  self._name = "debian_distribution"
  self._client = client
...
function DebianDistributionEntity:get_name()
  return self._name
end


function DebianDistributionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DebianDistributionEntity.new(self._client, opts)
end
...
function DebianDistributionEntity:mark_deleted()
  self._deleted = true
end


function DebianDistributionEntity:deleted()
  return true == self._deleted
end


function DebianDistributionEntity:data_set(args)
  if args ~= nil then
...
function DebianDistributionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DebianDistributionEntity:match_set(args)
  if args ~= nil then
...
function DebianDistributionEntity:match_get()
...
function DebianDistributionEntity:stream(action, args, callopts)
...
---@param reqmatch DebianDistributionRemoveMatch
---@param ctrl? table
---@return DebianDistribution
---@return string? err
function DebianDistributionEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function DebianDistributionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DebianDistributionEntity
