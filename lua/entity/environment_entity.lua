...
local EnvironmentEntity = {}
EnvironmentEntity.__index = EnvironmentEntity


function EnvironmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EnvironmentEntity)
...
function EnvironmentEntity:get_name()
  return self._name
end


function EnvironmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EnvironmentEntity.new(self._client, opts)
end
...
function EnvironmentEntity:mark_deleted()
  self._deleted = true
end


function EnvironmentEntity:deleted()
  return true == self._deleted
end


function EnvironmentEntity:data_set(args)
  if args ~= nil then
...
function EnvironmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EnvironmentEntity:match_set(args)
  if args ~= nil then
...
function EnvironmentEntity:match_get()
...
function EnvironmentEntity:stream(action, args, callopts)
...
---@param reqdata EnvironmentCreateData
---@param ctrl? table
---@return Environment
---@return string? err
function EnvironmentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch EnvironmentRemoveMatch
---@param ctrl? table
---@return Environment
---@return string? err
function EnvironmentEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function EnvironmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
