-- Gitlab SDK GroupAvatar entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GroupAvatarEntity = {}
GroupAvatarEntity.__index = GroupAvatarEntity


function GroupAvatarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GroupAvatarEntity)
  self._name = "group_avatar"
  self._client = client
...
function GroupAvatarEntity:get_name()
  return self._name
end


function GroupAvatarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GroupAvatarEntity.new(self._client, opts)
end
...
function GroupAvatarEntity:mark_deleted()
  self._deleted = true
end


function GroupAvatarEntity:deleted()
  return true == self._deleted
end


function GroupAvatarEntity:data_set(args)
  if args ~= nil then
...
function GroupAvatarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GroupAvatarEntity:match_set(args)
  if args ~= nil then
...
function GroupAvatarEntity:match_get()
...
function GroupAvatarEntity:stream(action, args, callopts)
...
---@param reqmatch GroupAvatarLoadMatch
---@param ctrl? table
---@return GroupAvatar
---@return string? err
function GroupAvatarEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GroupAvatarEntity:_run_op(ctx, post_done)
  local utility = self._utility
