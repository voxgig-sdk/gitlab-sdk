-- Gitlab SDK AccessRequest entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AccessRequestEntity = {}
AccessRequestEntity.__index = AccessRequestEntity


function AccessRequestEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AccessRequestEntity)
  self._name = "access_request"
  self._client = client
...
function AccessRequestEntity:get_name()
  return self._name
end


function AccessRequestEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AccessRequestEntity.new(self._client, opts)
end
...
function AccessRequestEntity:mark_deleted()
  self._deleted = true
end


function AccessRequestEntity:deleted()
  return true == self._deleted
end


function AccessRequestEntity:data_set(args)
  if args ~= nil then
...
function AccessRequestEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AccessRequestEntity:match_set(args)
  if args ~= nil then
...
function AccessRequestEntity:match_get()
...
function AccessRequestEntity:stream(action, args, callopts)
...
---@param reqmatch AccessRequestRemoveMatch
---@param ctrl? table
---@return AccessRequest
---@return string? err
function AccessRequestEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function AccessRequestEntity:_run_op(ctx, post_done)
  local utility = self._utility
